...
const MESSAGE_ROWS := [
	...
	["今日免费拼块次数已用完。本局需要 %d 金币。通关后 Good 奖励 %d 金币，Excellent 奖励 %d 金币。", "Today's free block rounds are used up. This round costs %d coins. Good awards %d coin; Excellent awards %d coins.", "انتهت جولات القطع المجانية اليوم. تكلف هذه الجولة %d عملة. تمنح نتيجة Good عدد %d، وتمنح Excellent عدد %d من العملات.", "Les manches gratuites du jour sont épuisées. Cette manche coûte %d pièces. Good rapporte %d pièce et Excellent %d pièces.", "Ludi tessellarum hodie gratuiti consumpti sunt. Hic ludus %d nummos constat. Good %d nummum, Excellent %d nummos reddit."],
