...
const MESSAGE_ROWS := [
	...
	["设置语言、音效和震动反馈。", "Set language, sound effects, and haptic feedback.", "اضبط اللغة والمؤثرات الصوتية والاهتزاز.", "Réglez la langue, les effets sonores et les vibrations.", "Linguam, sonos et vibrationem constitue."],
	...
	["开启", "On", "تشغيل", "Activé", "Activa"],
	["关闭", "Off", "إيقاف", "Désactivé", "Inactiva"],
	...
	["错误示范：这样会隔断同色区域", "Wrong example: this cuts off the matching color", "مثال خاطئ: هذا يفصل المنطقة ذات اللون نفسه", "Mauvais exemple : ceci coupe la zone de même couleur", "Exemplum pravum: hoc regionem eiusdem coloris separat"],
	["错误：同色区域已经无法连通", "Wrong: the matching color can no longer connect", "خطأ: لم يعد من الممكن وصل المنطقة ذات اللون نفسه", "Incorrect : la zone de même couleur ne peut plus être reliée", "Pravum: regio eiusdem coloris iam coniungi non potest"],
	["正确示范：把方块接到周围同色区域", "Correct example: connect the block to the matching color", "مثال صحيح: صِل القطعة بالمنطقة المجاورة ذات اللون نفسه", "Bon exemple : reliez le bloc à la zone voisine de même couleur", "Exemplum rectum: tessellam regioni vicinae eiusdem coloris coniunge"],
	["正确：方块与同色区域保持连通", "Correct: the block stays connected to its matching color", "صحيح: تبقى القطعة متصلة بالمنطقة ذات اللون نفسه", "Correct : le bloc reste relié à la zone de même couleur", "Rectum: tessella regioni eiusdem coloris coniuncta manet"],
	["棋盘已恢复，轮到你来放置", "Board reset. Now it’s your turn", "تمت إعادة اللوحة. حان دورك الآن", "Plateau réinitialisé. À vous de jouer", "Tabula restituta est. Nunc tu pone"],
	...
	["开始下一局？", "Start the next round?", "بدء الجولة التالية؟", "Commencer la manche suivante ?", "Proximum ludum incipiamus?"],
	["今日免费次数已用完", "Today's free rounds are used up", "انتهت الجولات المجانية اليوم", "Les manches gratuites du jour sont épuisées", "Ludi hodie gratuiti consumpti sunt"],
	["第 %d 局", "Round %d", "الجولة %d", "Manche %d", "Ludus %d"],
	["消耗", "Cost", "التكلفة", "Coût", "Sumptus"],
	["余额", "Balance", "الرصيد", "Solde", "Nummi"],
	["消耗 %d 金币", "Spend %d coins", "أنفق %d عملة", "Dépenser %d pièces", "%d nummos consume"],
	["当前持有 %d 金币", "Current balance: %d coins", "الرصيد الحالي: %d عملة", "Solde actuel : %d pièces", "Nunc %d nummos habes"],
	["开始", "Start", "ابدأ", "Commencer", "Incipe"],
	...
	["今日免费拼块次数已用完。本局需要 %d 金币。通关后 Good 奖励 %d 金币，Excellent 奖励 %d 金币。", "Today's free block rounds are used up. This round costs %d coins. Good awards %d coins; Excellent awards %d coins.", "انتهت جولات القطع المجانية اليوم. تكلف هذه الجولة %d عملة. تمنح نتيجة Good عدد %d، وتمنح Excellent عدد %d من العملات.", "Les manches gratuites du jour sont épuisées. Cette manche coûte %d pièces. Good rapporte %d pièces et Excellent %d pièces.", "Ludi tessellarum hodie gratuiti consumpti sunt. Hic ludus %d nummos constat. Good %d nummos, Excellent %d nummos reddit."],
