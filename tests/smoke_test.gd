...
func _run() -> void:
	...
	assert(ProjectSettings.get_setting("application/boot_splash/minimum_display_time") == 0, "Godot boot splash must not add a minimum delay")
	assert(ProjectSettings.get_setting("application/boot_splash/show_image") == false, "Godot boot splash image must stay disabled")
	assert(ProjectSettings.get_setting("application/boot_splash/stretch_mode") == 1, "Boot splash stretch mode must use the current Godot project setting")
	...
	assert(export_config.count("include_filter=\"\"") == 4, "Mobile exports must not force offline JSON or redundant imported SVG sources into packages")
	assert(export_config.count("data/*.json,docs/*,scenes/level_editor.tscn,scripts/level_editor.gd,scripts/tools/*") == 4, "Every mobile export must exclude offline data, documentation and editor-only resources")
	assert("keystore/release_password=" not in export_config, "Android release signing secrets must not be committed in export_presets.cfg")
	...
	assert(export_config.count("application/bundle_identifier=\"com.shingosuper.colorking\"") == 2, "iOS exports should share the production bundle identifier")
	assert(export_config.count("application/min_ios_version=\"15.0\"") == 2, "Godot 4.7.1 iOS exports should use the supported iOS 15 minimum")
	assert(export_config.count("application/export_project_only=true") == 2, "iOS exports should generate Xcode projects for local signing")
