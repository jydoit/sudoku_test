...
func _run() -> void:
	...
	assert(game.assembly_view._placement_feedback_cells.size() > 0, "Accepted assembly placement should trigger a board landing feedback")
	...
	assert(game.assembly_view._tray_feedback_slot_index == game.composite_tray_slots.find(history_piece_id), "Returned pieces should trigger a tray slot feedback")
	game._on_assembly_placement_requested(history_piece_id, history_origin)
