...
func _run() -> void:
	...
	var runner_arm: TextureRect = game.result_page._piece_texture_rect(Vector2.ZERO, game.result_page.LION_KING_CENTER_ARM_00)
	runner_arm.name = "ResultLionRunnerArm"
	runner.add_child(runner_arm)
...
func _capture_handoff_and_center_actions(_target: Vector2) -> void:
	...
	for arm_index in [0, 3, 6, 9, 12]:
		game.result_page._set_result_lion_arm_frame(
			game.result_page.RESULT_LION_WAVE_ARM_FRAMES[arm_index]
		)
		await _shot("center_arm_%02d" % arm_index)
