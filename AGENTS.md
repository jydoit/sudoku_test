...
- 正式关卡按展示关卡序号分配红心：第 1-10 关 3 个，第 11-30 关 2 个，第 31 关起 1 个；用户双击非标准答案格尝试放置皇冠时，扣除 1 个红心并把该格标记为红色 X，红心为 0 时进入关卡失败页。
- 每个用户初始有 3 次皇冠直找次数；点击皇冠按钮会直接找到一个标准答案皇冠，免费次数用完后显示金币价格，后续可通过金币或激励广告入口补足。
- 清除是永久免费的关卡道具，可清除普通 X、普通皇冠和错误红色 X；保留开局提示皇冠和皇冠直找生成的锁定皇冠，不返还红心、免费道具次数或金币。
...
- 正式关卡 Hint 的可操作范围只能包含可画普通 X 的非皇冠空格，不能包含标准答案皇冠格、候选观察格或推理单元；范围内每格必须可单击画 X，没有合法目标时不消耗提示次数。新手教程按独立教学状态机处理。
- 正式关卡提示只通过棋盘图像差异和可操作范围表达，不显示解释文字或成功提示 Toast。
- 提示和新手引导通过整盘 `70%` 冷灰蒙层区分可操作范围：区域边界随蒙层统一变暗但不隐藏，全部提示相关格和已有 X/皇冠重绘为正常亮度；主要操作格允许播放一次柔和光环，但不加实线高亮框、不循环闪烁；其它空格禁止点击。
...
- 关卡失败页沿用关卡结果页蓝色整屏结构，底部按钮为“金币复活”和“重新挑战”；金币不足时显示购买金币、观看激励广告和稍后再说入口，不自动播放广告。
- 棋盘颜色区域需要兼顾色弱和低色彩敏感用户：不同颜色区域之间和棋盘最外缘用深色边框，同色区域内部使用同色加深缝隙，确保不靠颜色也能看出区域轮廓。
