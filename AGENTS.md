...
- 核心棋盘交互必须区分普通排除 X、普通皇冠、提示皇冠和错误红色 X；普通 X 可取消，普通皇冠可继续调整，提示皇冠和错误红色 X 锁定。
- 每个正式关卡有 3 个红心；用户双击非标准答案格尝试放置皇冠时，扣除 1 个红心并把该格标记为红色 X，红心为 0 时进入关卡失败页。
- 每个用户初始有 3 次皇冠直找次数；点击皇冠按钮会直接找到一个标准答案皇冠，次数用完后禁用，后续预留激励广告补充次数。
- 棋盘必须支持滑动批量处理普通排除 X：从空格开始滑动时连续标记普通 X；从普通 X 开始滑动时连续取消普通 X；滑动不得标记皇冠，也不得改变皇冠。
- 棋盘颜色必须使用产品文档固化的“明亮经典”10 色默认色板；相邻区域不能使用相近颜色。
- 棋盘不能只依赖颜色区分区域；棋盘底色为白色，不同颜色区域边缘和棋盘最外缘使用黑色边框，同色区域内部不画边框。
...
- 提示和新手引导只通过灰色浮层区分可操作范围：全部提示相关格和已有 X/皇冠保持正常亮度，不加高亮框、不闪烁；其它空格覆盖透明灰色浮层并禁止点击。
...
- 首页和关卡结果页使用统一的蓝色主色系；首页优先呈现 `color king` 主视觉、当前关卡入口和新人流程入口。
- 首页和关卡页职责分离：金币、排行榜等资源与社交入口不在关卡页展示；关卡页优先服务解题。
- 关卡结果页使用整屏奖励结构：顶部完成文案、中部皇冠展位、底部“下一关”和“主菜单”；当前不显示排行榜、奖励进度栏和广告位。
- 关卡失败页沿用关卡结果页蓝色整屏结构，底部按钮为“重新挑战”和“返回首页”。
- 棋盘颜色区域需要兼顾色弱和低色彩敏感用户：不同颜色区域之间和棋盘最外缘用黑色边框，同色区域内部不画边框，确保不靠颜色也能看出区域轮廓。
