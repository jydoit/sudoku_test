...
- 结算页只展示真实产生的金币奖励，不展示没有后续用途的“获得皇冠 +1”虚拟奖励；奖励行只保留金币图标和右侧金色大号账户余额，不再显示 `coins / 金币奖励` 文字。关卡顶部余额字号为 `24px`，数字区最小为 `62 x 44px`；结算页余额字号为 `32px`，数字使用同色细描边加粗，数字区最小为 `104 x 52px`并保留上下字体安全空间，多位数时继续自动增宽。结算金币图标使用 `44 x 44px`，与数字间距约 `3px`；数字在预留的五位数裁切区内向金币侧对齐，短余额不能因居中留白而与图标分离。页面淡入后停顿约 `0.45s`，数字先显示本次奖励到账前的账户余额；小狮子配合挥手/抛金币动作，将与奖励数量一致的金币沿交错弧线逐枚抛向奖励行的金币图标，每枚金币落入时只触发目标图标的轻微回弹，余额在全部飞币完成前保持初始值。最后一枚金币完全落入后，余额使用单个 Tween 启动一次滚筒动画：只展示少量抽样中间值，不逐金币排队计数，前段快速掠过、末段减速并精确停在用户当前持有的金币总数。大额消费仍最多构建 `10` 个可见滚动步骤，在保持结束值和总时长正确的同时避免大量 Tween 导致手机掉帧。飞行金币固定使用移动端小尺寸，不能被素材原始尺寸撑大或飞出结果页。滚筒停止后小狮子恢复随机庆祝动作，整段结算动画按奖励数量控制在约 `2.15-3.3s`；金币已在结算状态写入时落盘，动画不得延迟或重复发放。
