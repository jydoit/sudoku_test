...
- 发布前，以“回归测试清单”为准逐项验证。
- 游戏 UI、Godot Control、动画和性能优化的统一落地方式见 [`docs/GAME_UI_SKILL_INTEGRATION.md`](GAME_UI_SKILL_INTEGRATION.md)；该文档不得覆盖本文档中的玩法和产品事实。
...
- iOS 保留系统要求的原生 Launch Screen，但 Debug 与 Release 导出预设必须显式使用与现有蓝色背景完全同色、完全不透明的占位图；不得使用可能被 Xcode 编译成模板资源的全透明图片，也不得留空回退到 iOS 导出模板自带的 Godot Logo。Godot 导出后必须运行 `tools/prepare_ios_export.sh`，依据 build number 将模板固定的 `Launch Screen` storyboard 改为 color king 专用的版本化缓存键。原生 Launch Screen 结束后直接衔接游戏内“拼块成盘”Splash；修改原生启动资源时必须递增 iOS build number，使脚本生成新的缓存键，避免系统继续复用旧启动快照。
...
- 竖屏移动端以 `540x960` 为基准检查，内容位于安全区内，狮子和色块不得越过屏幕边缘；横屏或窄屏时只缩放内容，不改变动画顺序。
- 尊重静音、音效音量和减少动画设置。减少动画模式下取消闪光、缩放回弹和色块飞行，仅保留淡入、短暂停留和淡出；允许用户在首个稳定画面后跳过。
...
关键帧资源：图像生成模型产出的 4x4 位图母版仅保存在 `docs/animation_sources/splash/`，由 `tools/process_splash_sprite_sheet.py` 去除烘焙棋盘格、统一注册并逐帧矢量追踪；运行时只加载 `assets/ui/splash/splash_assembly_00.svg` 至 `15.svg` 共 `16` 张纯路径 SVG，并在最终稳定帧叠加项目标准 `lion_king_center_body.svg`，不加载或打包逐帧 PNG。SVG 画布统一为 `320x320`，不得包含 `<image>` 嵌入、文字、水印或真实关卡答案。
...
- 真实资源已就绪时正常模式总时长约 `6.00s`、减少动画模式约 `2.10s`；资源未就绪时不显示空白页面、不循环跳动，准备完成后从稳定画面自然退出。
- 动画中途退出应用不会写入教程完成、关卡进度、金币、红心、皇冠或拼块现场；重新启动可以正常继续原流程。
...
- `assets/icon.svg` 是商店、旧版启动器和不支持自适应图标的平台使用的完整合成图，蓝天渐变背景必须铺满且完全不透明，由平台负责圆形、圆角方形等最终蒙版裁切。
- `assets/icon_foreground.svg` 是 Android 自适应图标透明前景，只保留拼块皇冠、小狮子和必要的对象投影；透明区域不得被白色或蓝色矩形烘焙。生成时必须把系统识别出的皇冠与狮子保存为两个独立矢量组：皇冠固定应用 `translate(333 221) scale(0.546)`，使完整外轮廓进入安全圆；狮子固定应用 `translate(261 169) scale(0.7)`，使完整面部和伸出的手掌在圆形与圆角方形蒙版中可辨认，并让母版底部裁切线继续隐藏在系统蒙版外。两项变换不得反向影响商店与旧版启动器使用的完整合成图。
- `assets/icon_background.svg` 是与完整合成图一致的全不透明蓝天渐变背景，不包含狮子或皇冠，避免启动器视差和主题蒙版移动时露出透明边缘。
- `assets/icon_monochrome.svg` 是 Android 主题图标使用的单色透明前景，使用单一实色轮廓并保留必要的眼睛、笑脸和拼块负形；不得直接把彩色图降透明度充当单色层。
- 四份 SVG 共用 `0 0 1254 1254` 的母版视图坐标和同一套源对象比例；彩色与单色自适应前景分别对皇冠和狮子应用一致的双组安全区构图，背景和完整合成图保持母版坐标。Android 三层资源使用 `432 x 432` 固有导出尺寸，完整合成图使用 `1024 x 1024`。Android Debug 与 Release 导出预设必须分别接入同一组前景、背景和单色资源。主识别内容按 Android `108 x 108dp` 自适应图标坐标检查：皇冠完整轮廓以及狮子的眼睛、口鼻和手掌必须落在中央 `66 x 66dp` 安全圆内，只有角色身体外围允许延伸到蒙版裁切区；蒙版预览必须组合真实的前景与背景层，并按 `108dp` 图层画布、`72dp` 可见蒙版范围验证圆形、圆角方形、Squircle 和启动器缩放后的裁切效果，不能使用完整合成图代替分层预览。
- `tools/extract_app_icon_foreground.swift` 使用系统前景实例蒙版从已确认母版中分离皇冠和小狮子，按实例垂直质心确定上方皇冠与下方狮子并分别输出；`tools/build_app_icon_vectors.py` 再通过固定参数把合并主体、皇冠和狮子分别转为纯路径 SVG，以双组构图生成自适应前景，同时用合并主体保持完整合成图不变，并以确定性渐变和光芒重建背景。参考 PNG 和全部中间提取图不得嵌入最终 SVG，也必须从 Debug、Release 包体中排除。
...
- 首屏显示与关卡结果页一致的蓝色主色系背景，并与启动 Splash 复用同一份 `color_king_title.svg` 品牌主视觉；标题必须保留圆润粗体、金色渐变、深金描边、蓝灰阴影和矢量闪光，不得回退为普通 Label。
...
- “拼块玩法”按钮在完成新人流程后可点击，并与普通关卡的 6x6 size 共用同一个解锁条件：第 30 关同时开放 6x6 普通关卡和拼块玩法。未达到条件时入口显示“第 30 关解锁”，点击弹窗提示所需关卡且不创建拼块现场；达到后进入独立的 6x6-9x9 复合拼块挑战。进入前保存主线关卡、资源与未完成棋盘，返回首页时完整恢复主线关卡现场；独立体验不增加主线关卡号、通关记录或主线导演统计，但拼块入场费和通关金币会同步保留到共享金币余额。每局先调用普通玩法推荐核心，从已解锁且存在离线拼块数据的棋盘中得到 `size × difficulty class`，再由独立拼块导演为该 size 采样 Simple/Medium/Hard 拆分 pattern。标题显示“拼块挑战 · 第 N 局”，不在关卡标题右侧展示抽中的 pattern。独立玩法另外保存最近局数、推荐模型、pattern 后验、每日免费次数、入场费、应发奖励、seed、拼块位置、放置顺序、死局状态、红心、道具余量和皇冠阶段棋盘；退出仍恢复主线现场，再次进入时恢复未完成局且不重复计次或收费，最近一局已完成则进入下一局。

当前实现：

- 首页 UI 由独立的 `scripts/pages/home_page.gd` 构建并持有共享 SVG 品牌标题及“开始关卡 / 拼块玩法 / 新人流程”三个入口；页面只接收展示数据并发出语义化信号，不读取关卡、存档或经济状态。
...
- 关卡 UI 按玩法拆为两个独立页面实例：`scripts/pages/formal_level_page.gd` 承载常规皇冠玩法，`scripts/pages/composite_level_page.gd` 承载拼块玩法及其完成拼接后的皇冠阶段；两者共享 `scripts/pages/level_page_base.gd` 中的顶部栏、关卡标题、进度、棋盘、工具栏、红心与金币动画。顶部栏和结算页的金币图标、数字裁切窗口及滚动逻辑统一由 `scripts/components/coin_roll_display.gd` 提供；组件按字体实际高度和余额位数计算裁切尺寸，至少完整承载五位数，静态更新会终止旧 Tween 并复位备用数字，且在 RTL 页面中仍固定使用“金币图标 + 数字”的 LTR 顺序。`scripts/main.gd` 只切换当前页面并通过页面 API 更新金币、进度和红心，不再在页面切换时复制绑定整套内部节点引用。
...
- 皇冠直找按钮与清除、提示按钮同一行显示；每个新用户初始 1 次皇冠直找和 2 次提示，点击后直接把一个未找到的标准答案格标记为锁定皇冠。三个按钮统一使用紧凑的“图标 + 简短名称 + 固定高度状态胶囊”结构：清除显示“免费”，直找和提示有次数时显示“免费 ×N”；次数为 0 时不显示 `×0`，状态胶囊直接切换为金币图标和本次价格。付费胶囊必须使用不透明底色：直找沿用金币黄体系，提示使用提示绿体系，不能只依赖透明金币贴图承载价格；金币图标使用 SVG 矢量源，避免小尺寸道具价格出现位图毛刺。工具栏不得挤压棋盘或在 540×960 竖屏下越过底部安全区。
...
- `play_cell_feedback()`、`play_wrong_feedback()` 和 `play_guide_feedback()` 分别提供普通放置/标记弹跳、错误皇冠短促晃动和提示柔和光环反馈。每类临时反馈必须由唯一、可取消的 Tween 持有；切关或开始整盘胜利时间轴时先取消单格弹跳、错误晃动、表情反应和皇冠揭示，最终皇冠不能同时叠加“正确放置”和“整盘庆祝”两套动作。
...
- 免费提示初始为 5 次。
- 免费次数用完后，按钮主标签保持“提示”，不显示 `×0`。底部固定状态胶囊始终由“免费 ×N”切换为小金币图标和当前动态价格，无论当前金币余额是否足够都保持同一价格表达；余额不足只在玩家点击后通过金币不足弹窗反馈，由玩家确认购买金币、观看激励广告或稍后再说，不自动播放广告。进入或恢复任意关卡、页面切换以及棋盘状态校验后都必须以当前免费次数和动态价格重新计算该胶囊，不能保留页面创建时的 `Free` 占位状态。
...
- 离线事实源：`data/levels.json`；该 JSON 只用于关卡生成、编辑和 review，不在游戏启动时解析。
- 真机运行目录：`data/runtime/level_catalog.res`。通过 `scripts/tools/build_runtime_level_bundles.gd` 在发布前从 JSON 生成，数值类型、默认字段与结构校验全部在离线构建阶段完成。
- 加载器：`scripts/level_store.gd` 只读取二进制运行目录，不再对关卡树逐格标准化，也不复制整份解析树。
...
- 动态 size 解锁节奏由 `SIZE_UNLOCK_DISPLAY_LEVELS` 统一控制：第 1 关开放 5x5，第 30 关起开放 5x5-6x6 并同步解锁拼块入口，第 80 关起开放 5x5-7x7，第 160 关起采样 6x6-8x8，第 240 关起采样 6x6-9x9。
...
1. `assembly_intro`：首次体验时进入一次自动对比教学。系统先自动演示一个“空间可放、但会隔断同色区域”的错误落点，用红色断连反馈说明该摆法已无法完成；复位后再自动演示一个与周围固定同色格上下左右相接的正确落点。两次示范结束后必须把棋盘和托盘恢复到本局完全未放置的初始状态，再解除输入锁定交给玩家。教学可跳过，返回首页会取消教学且不会误记为已完成；已经看过时直接进入拼接，Help 中可重播但不修改历史完成状态。
...
- 拼块首次引导使用只作用于视图的自动拖放示范，不向 `CompositeGameController` 提交真实放置，也不写入中间存档。错误示范的落点必须通过共用规则验证为“放置后同色已无法连通”，正确示范的落点必须属于完整合法布局且至少邻接一个固定同色格，不能由 UI 伪造结论。引导期间保留“返回首页”和引导内“跳过”，临时禁用棋盘、托盘、Help、设置、选关及底部道具；完成或跳过后统一清空示范层并从初始现场开始。
...
- 已锁定颜色区域使用原“明亮经典”颜色，并复用中性灰白的单格立体砖贴图 `assets/ui/block_tile_neutral.svg`；运行时以区域颜色乘色，统一得到圆角顶面、左上高光、下沿厚度和柔和投影。锁定区域不可点击、拖动或旋转。
...
- 离线拼块数据生成器位于 `tools/generate_composite_levels.py`，直接读取 `data/levels.json`，完全在 Python 内执行区域选择、线索筛选、模板拆块、动态 MRV Exact Cover、皇冠唯一解求解和放置顺序生成，不启动 Godot 或调用 GDScript。生成结果写入 `data/composite_levels.json`：当前为 2000 个 6x6-9x9 基础关卡各生成 Simple/Medium/Hard 三档，共 6000 条；5x5 不进入复合拼块。发布前再由 `scripts/tools/build_runtime_level_bundles.gd` 把 JSON 转为 `data/runtime/composite_manifest.res` 和按 6x6、7x7、8x8、9x9 划分的四份完整 size 二进制运行目录，并离线写入施工索引、固定线索索引和候选落点缓存；不得再把每个 size 拆成多个运行时缓存分片。运行时只读取轻量 manifest；加载存档后优先异步加载当前最适合分发的完整 size，首页显示后以后台线程错峰预加载其它已解锁 size，并在距下一 size 解锁两关时提前预加载；仅恢复未完成拼块局时同步等待对应 size。推荐器始终依据 manifest 中完整的 6000 条可用性索引选择关卡，不会因为尚未加载某个 size 而缩小推荐候选；真正进入关卡时才取得对应 size 数据。运行时不再在线拆分、逐格标准化或 `duplicate(true)` 整份拼块数据。缺少二进制条目时关闭本关拼块入口。线索点从非割点安全边界格中按 `max(行异色格数, 列异色格数)` 取最高分，同分时只按固化 seed 选择。离线唯一性只统计同时满足“每块一次、无重叠、填满施工区、方块颜色逐格匹配原始 `regionId`”的完整方案；同颜色、同形状、同方向的视觉相同块只交换内部编号时视为同一方案。每个颜色区域拆分后先独立证明区域铺法唯一，再进入整局搜索；整局搜索使用占格位掩码、剩余等价块组和候选下界组成的记忆化状态，结果截断为 `0/1/2+`。切分期按候选落点数量、重复形状和强制剥离链质量排序，并允许依据第二解对歧义块做同色连通边界局部修正。只有搜索空间耗尽、合法放置方案唯一、恢复布局逐格等于输入且皇冠答案唯一时才允许输出，具体流程见 `docs/COMPOSITE_OFFLINE_GENERATION.md`。
- `scripts/composite_level_director.gd` 独立控制首页拼块玩法出关：内部调用 `LevelDirector.recommend_level_for_sizes()`，复用普通玩法的 size/difficulty 冷启动、曝光配额、Beta 与 Dirichlet 推荐逻辑，但把统计写入独立的 `compositeDirectorProgress.levelRecommendationProgress`，不污染主线。第 1-4 局使用 `Simple → Medium → Simple → Medium` 开局循环并禁止 Hard；某个基础关卡缺少目标 pattern 时只在 Simple/Medium 中回退。第 1 局还必须从离线数据中选择恰好 2 个可移动拼块、并同时具备真实错误断连位置与正确邻接位置的关卡。第 5 局起，基础关卡确定后再在该 size 下对 Simple/Medium/Hard 建立多项分布；冷启动随机探索概率为 50%，随着总样本达到 30 且每个 pattern 至少达到 6 次而线性降到 20%。探索分支均匀随机，非探索分支按 Dirichlet 后验权重进行多项采样。通关时增加当前 pattern 后验质量，失败时按普通导演相同的负反馈公式把证据分配给其它 pattern。
- `scripts/composite_coin_policy.gd` 独立控制拼块玩法的每日免费、入场费和通关金币。每个本地自然日前 3 个成功创建的新局免费，恢复未完成局或重玩当前失败局不重复计数或收费；第 4 个新局起，每局固定消耗 2 金币。拼块皇冠阶段未发生错误放置时评为 `EXCELLENT` 并返还 4 金币；发生过错误放置时评为 `GOOD` 并返还 2 金币。免费局同样按最终表现发放对应通关奖励。本地日期变化时免费次数重置，首页和“下一局”按钮在付费局开始前展示价格；从共享成功结算页点击付费“下一局”时，必须先在结算页上方显示统一确认弹窗，用户确认后才创建新局，取消则停留在原结算页。弹窗采用“标题 + 最多两行的简短说明 + 局数胶囊 + 消耗/余额双数据卡”结构；局数、消费数和余额使用大号加粗数字，消费与余额数据卡都使用共享金币图标，不再把数字塞进长段正文或主按钮。余额不足时拦截且不创建现场。拼块成功结算必须复用普通玩法的同一个 `ResultPage` 实例、布局和庆祝流程，只按拼块语义替换局数与金币说明。付费局结算文案同时展示入场扣除、通关奖励与本局金币净变化，因此 Good 付费局净变化为 0，Excellent 付费局净增加 2 金币。付费新局成功进入关卡后，关卡页首帧保持扣除前余额，再由顶部金币余额滚动到扣除后，并在金币徽章下方浮出本次扣除数量；不得在结算页或页面切换前提前播放，恢复未完成局也不得重播扣费动画。
...
- `scripts/assembly_view.gd` 使用 `assets/ui/block_tile_neutral.svg` 作为可运行时染色的单格纹理，拼装立体锁定区域、托盘方块、拖动预览和棋盘落块；同时绘制凹槽、深色横向待放置区、颜色发光 slot、提示浮窗、合法放置后的落点光环、撤回到托盘后的 slot 回弹反馈和压平过场，并处理鼠标与单指触控的横向滚动/拖放仲裁。
- `scripts/level_director.gd` 在 6x6 及以上、展示关卡为 10 的整数倍的里程碑挑战日程中写入 `assemblyEnabled`；所有实际挑战关都清空 `kingPositions`，拼块期间不安排开局提示皇冠。
...
- `scripts/pages/result_page.gd` 是独立的全屏结果页，同时承载成功、失败、拼块死局和教程完成四种视觉状态。页面通过 `present_success / present_failure / present_deadlock / present_tutorial_complete` 接收语义数据，自行生成本地化文案并管理金币飞入、余额滚动、小狮子庆祝和花瓣动画；奖励计算与完成/失败统计由 `scripts/services/run_result_service.gd` 负责，`scripts/main.gd` 只处理结算后的导航。成功结算进入时，不绘制骨骼化身体或四肢，也不播放整只狮子跑步。系统在左侧、右侧和底部三个物理屏幕边缘中等概率选择入场方向；左、右方向各使用 `16` 张独立逐帧 SVG，底部方向使用 `lion_bottom_entry_00.svg` 至 `10.svg` 的 `11` 张探头/支撑帧，以及 `lion_bottom_jump_00.svg` 至 `08.svg` 的 `9` 张独立起跳帧。生成母版保存在 `docs/animation_sources/lion_left_peek_jump_sheet.png`、`lion_right_peek_jump_sheet.png`、`lion_bottom_peek_jump_sheet.png` 和 `lion_bottom_jump_keyframes_sheet.png`。
- 探头阶段约 `1.55s`：前 `7` 个身体姿势在约 `52%` 时间内依次展示皇冠、眼睛、双爪、完整头部、鬃毛和上身，随后进入观察停顿。底部、左侧和右侧观察段都只眨眼 `1` 次，闭眼保持约两个采样步后重新睁眼；观察段始终保留各方向同一张睁眼身体底图和同一屏幕边缘注册位置，只允许叠加 `lion_bottom/left/right_peek_blink.svg` 眼部透明覆盖层，手爪、头部、鬃毛、身体高度和控件位置均不得随眨眼变化。眨眼覆盖层结束并重新睁眼后，才允许第 `9-11` 帧进入独立蓄力阶段，让双爪持续压住对应屏幕边缘并只完成一次重心下沉。底部起跳的 `9` 帧必须按“压低后开始伸展 → 双手离边 → 身体持续上升 → 仅一次收腿 → 双臂举高并自然展开 → 双腿向下准备落地”单向推进，离边后禁止再次回到下蹲或坐姿，手臂也不得出现“下垂—举高—再下垂—再举高”的反复。底部腾空帧在约 `0.48s` 内播放，逐帧姿势约 `18fps`；三个方向的位置轨迹均继续按每个渲染帧沿贝塞尔曲线插值。
- 三个方向都禁止复用头像、透视 Shader 或控件横纵缩放来冒充姿势变化，支撑阶段双爪必须锁定物理屏幕边缘，离边后才前往角色展位中心。边缘接触帧与居中腾空帧的注册差必须在起跳首帧按方向补偿，并在跳跃前 `28%` 路程内平滑释放，不能让角色刚离边就横向或纵向闪移。小狮子到达展位后，入场层只在落地阶段完成唯一一次位置缓冲，并从边缘展示尺寸连续缩放到展位的 `164px` 尺寸；接触展位后必须清空方向入场纹理，并立即切换为一张不透明的完整中央狮子位图，禁止在运行时叠加独立身体和手臂层，也禁止透明混合或形变插帧，以免产生断臂接缝、第二只手臂、尾巴重影、闪动或肢体伸缩。落地结束后的抵达庆祝必须锁定角色中心坐标、展位尺寸和 `0` 旋转，只允许切换同一注册画布的完整角色帧，不得再次上下位移、旋转或缩放身体。
- 中央角色使用图片生成模型绘制的完整角色位图：`lion_king_center_full_wave_00.png` 包含已经烘焙为一个整体的头、皇冠、鬃毛、身体、双臂、双腿和尾巴，并使用 `400 x 400` 透明注册画布；运行时只能显示一张完整纹理，不存在固定身体底图或独立手臂叠层。角色从屏幕边缘完成落地并切换到中央展位后，必须立即固定为第 `0` 张微笑画面；抵达交接过程、等待奖励和奖励结束后都不得播放挥手、欢呼、调皮、呼吸缩放或其它循环动作。发放真实金币奖励时切换到专用完整角色帧 `lion_king_center_coin_toss_00.png` 至 `06.png`，约 `0.84s` 单向播放“腹前双手捧住金币堆 → 胸前托起 → 双手向上送出 → 双臂同时举至皇冠两侧 → 金币向头顶上方爆开 → 空手向外回收 → 双手落回胸前”；含金币的蓄力帧禁止倒放，不能让金币飞回手里。七帧的头、皇冠、鬃毛、躯干、腿和尾巴必须锁定同一注册位置，双肩和双臂烘焙在完整角色中，禁止单臂挥手、运行时拼接或按奖励数量拉伸动作时长。模型母版保存在 `docs/animation_sources/lion_center_coin_toss_model_sheet.png`，由 `tools/extract_lion_center_coin_toss_frames.gd` 离线去除模型返回的 RGB 棋盘格并输出透明注册帧。EXCELLENT 在花瓣结束前约 `0.34s` 提前进入捧币蓄力，消除花瓣结束后的静态停顿；动作开始约 `0.37s`、双手达到头顶释放姿势时，同步生成 `12` 枚 `20px` 装饰金币沿更高、更宽的上抛扇形轨迹散开并淡出，释放仍发生在花瓣完全结束之后。装饰金币须采用确定性的错层参数：在约 `0.08s` 内分批起飞，最高点上下差至少约 `58px`，同时改变飞行时长、前后缩放层级、落点和旋转，使空中分布自然错落而非排列成平齐横线；装饰金币不改变真实奖励数量、余额和到账音效。动作结束立即回到第 `0` 张微笑画面永久静止，不得继续挥手。中央帧动画器只注册 `coin_toss` 这一条动作轨道；wave/cheer/playful 轨道已经从运行时代码移除，原 `lion_king_center_full_wave_01.png` 至 `06.png` 文件仅作为待确认删除的历史素材保留且不再预加载。肩部必须与躯干连续连接，禁止断臂接缝、独立肢体层、直角钩形、柱状手臂或运行时形变。旧的中央固定身体 + `lion_king_center_arm_00.svg` 至 `12.svg` 独立手臂、`lion_king_center_landing.svg` 以及本分支已拒绝的独立曲肘位图方案均标记为废弃；历史资源保留待确认删除，但不进入当前结算页运行逻辑。GOOD 的飞币由入场完成事件触发，不使用固定时长计时器；EXCELLENT 的入场可与撒花同时进行，双手捧币蓄力允许与最后 `0.34s` 花瓣淡出重叠，但装饰金币和真实金币仍须在花瓣结束后释放。
- 结算页只展示真实产生的金币奖励，不展示没有后续用途的“获得皇冠 +1”虚拟奖励；奖励行只保留金币图标和右侧金色大号账户余额，不再显示 `coins / 金币奖励` 文字。关卡顶部余额保持 `24px` 和至少 `62 x 44px` 的紧凑固定布局；结算页余额改为按实际位数自适应：一至六位分别使用 `38/36/34/32/29/26px` 字号以及至少 `42/58/76/94/108/122px` 数字区宽度，七位及以上使用 `23px` 并依据字体实测继续扩宽。结算金币图标使用 `44 x 44px`；金币图标和数字裁切区之间必须插入独立的 `28px` 布局占位节点，不依赖 HBox 主题 separation，数字区左侧另保留 `8px` 内边距，使图标控件右边缘到数字字形起点至少约 `36px`。奖励组保持整体居中，使金币相对向左、数字相对向右展开。滚动动画开始前按 `max(初始余额位数, 目标余额位数)` 一次性锁定本轮字号和数字区宽度，滚动过程中不得改变布局；动画停止后才允许按最终余额位数收紧，例如 `99→100` 全程使用三位规格，`100→99` 停止后才收紧为两位规格。通关奖励可以立即写入钱包和存档，但关卡页顶部必须继续显示奖励前余额，禁止在局内完成庆祝阶段提前展示增加；只有结算页负责可见的到账过程。结算页必须接收钱包交易返回的明确到账前、到账后余额，并在奖励行首次显示前写入到账前余额，首帧不得显示组件默认的 `0`、上一局残留余额或已经到账后的余额；页面淡入和 Excellent 撒花期间都保持该奖励前余额。GOOD 在入场结束后立即启动约 `0.84s` 的双手托举抛币动作；EXCELLENT 则在花瓣结束前约 `0.34s` 开始同一蓄力动作，并在动作开始约 `0.37s` 的头顶释放点、花瓣已经结束后，同时撒出装饰金币和第一枚真实奖励金币。与奖励数量一致的真实金币继续沿交错弧线逐枚飞向奖励行金币图标，角色不等待所有金币抵达才收手。数字宽度更新后必须等待容器完成布局，再以金币图标的实际全局中心建立飞行终点，禁止使用旧布局位置或奖励行边界作为接收焦点；每枚金币落入时只触发目标图标的轻微回弹，余额在全部飞币完成前保持初始值。最后一枚金币完全落入后，余额使用单个 Tween 启动一次滚筒动画：只展示少量抽样中间值，不逐金币排队计数，前段快速掠过、末段减速并精确停在用户当前持有的金币总数。大额消费仍最多构建 `10` 个可见滚动步骤，在保持结束值和总时长正确的同时避免大量 Tween 导致手机掉帧。真实飞行金币固定使用移动端小尺寸，不能被素材原始尺寸撑大或飞出结果页。抛币动作结束后小狮子立即恢复微笑静止画面，数字滚筒停止后也不再追加挥手或其它角色动作；整段结算动画按奖励数量控制在约 `2.15-3.3s`。金币已在结算状态写入时落盘，装饰散币不得写入经济系统，动画不得延迟或重复发放。
- 当前飞币起点不再使用固定 `0.45s` 页面计时：GOOD 等待小狮子入场完成；EXCELLENT 在小狮子已入场的前提下，于花瓣结束前约 `0.34s` 提前进入双手捧币蓄力，花瓣结束后立即衔接金币释放。
...
- 顶部设置入口使用 `assets/ui/settings.svg` 纯路径齿轮图标，不依赖 Unicode `⚙` 字形；图标必须在 `46×46px` 点击区内水平、垂直居中，并使用清晰的 `5px` 深蓝描边，在 iOS、Android 和编辑器中保持一致可见。
- 顶部帮助入口使用 `assets/ui/help.svg` 纯路径问号图标，不依赖字体中的 `?` 字形；图标同样在 `46×46px` 点击区内居中，并使用 `5px` 深蓝描边。
- 顶部返回首页入口使用 `assets/ui/home.svg` 纯路径房屋图标，不依赖 Unicode 房屋字符；图标必须在 `52×52px` 点击区内居中。首页、帮助和设置三个图标按钮在 iOS、Android 与编辑器中使用相同的居中规则。
...
- `assets/ui/` 不存在 PNG 游戏素材；所有 SVG 均可被 Godot 正常导入，且源码不包含 `<image>`、`data:image` 或其它内嵌栅格数据。
...
- 默认采用“木质玩具 + 轻金属 + 纸面”的声音语言：普通 X 使用约 `0.125s` 的低亮度软金属落笔声，取消 X 使用约 `0.185s` 的轻纸面擦除声；两者保持接近的感知响度、短尾音和轻微随机音高，形成“落下 / 撤回”的同组操作反馈，不得一个尖锐、一个拖沓。清除保留多次橡皮往返摩擦纸张声；正确皇冠、错误、红心损失、提示和通关使用柔和木质敲击音。
- 普通高频操作保持轻且短；正确皇冠随本关已找到数量小幅提高音高，形成进度感。错误皇冠与红心损失合并为一个先错误、后低沉心跳的连续音效，不得同时叠放两个高音量素材。
...
- 拼块整盘完成音效与 `AssemblyView.play_flatten_transition()` 同时开始，包含落位、沉降和转换闪光层；最终直找补齐整盘时同样必须执行该完成音效。压平 Tween 由 `AssemblyView` 独占，重新配置、离开页面或切换玩法时必须取消；主流程只在 Tween 正常完成且仍处于 `transition` 阶段时进入皇冠阶段。
- 第一轮素材位于 `assets/audio/candidates/`，映射说明位于 `assets/audio/README.md`，可由 `tools/generate_audio_sfx.py` 确定性重新生成。
- `scripts/audio_controller.gd` 统一管理音量、随机变体、滑动标记/擦除/拼块吸附节流和并发播放；页面脚本只调用语义化播放方法，不直接散落加载与播放逻辑。运行时使用普通操作、UI、庆祝三组独立播放器池，优先复用空闲播放器，避免高频操作截断庆祝尾音；庆祝音播放期间普通操作总线暂时降低约 3.5 dB，并在尾音结束后平滑恢复。Master 总线运行时安装 `-1 dB` ceiling 的 Limiter，避免组合音效产生削波。
- 当前接入事件包括普通 X、取消 X、滑动批量标记与取消、进度化正确皇冠、错误皇冠与红心合并反馈、提示、清除、皇冠直找、通关，以及拼块拿起、吸附、落位、拒绝、退回、区域完成、死局、复活、整盘清除和压平转换。
...
- iOS、Android 和编辑器中的短促操作音效不得出现由有损导入导致的砂噪、前回声或瞬态变形；发布包需确认除 `result_cheerful.wav` 外的候选 WAV 均以 PCM 导入，结算音乐继续使用 QOA。
- 拼块只有在越过拖动阈值后播放拿起音；同一吸附点停留不重复播放，切换到新合法点才播放下一次。
...
- 正确选择皇冠时，小狮子在“微笑双弹”和“微笑摇摆庆祝”中随机选择一种约 `0.72s` 的连续动作并出现短暂金色亮点；位移、缩放和旋转按每个渲染帧插值，头像由普通微笑平滑过渡到弯月笑眼、嘴角上扬的开心表情，短暂停留后回到稳定皇冠头像，不得闪切。错误选择时，已有委屈表情必须在红色 X 上方清晰停留后淡出，并以约 `0.82s` 的进入、歪头下沉、退场节奏和扣心反馈同步，不能只显示红 X 或只左右抖动。
- EXCELLENT 结算播放延迟加载的柔和欢快音乐 `result_cheerful.wav`，旋律使用低亮度木质与卡林巴音色，禁止尖锐玻璃高频；GOOD 结算不播放该音乐。EXCELLENT 使用 32 枚花瓣、总时长约 `4.8s`。花瓣统一使用可运行时染色的白色中性矢量素材 `assets/ui/petal_neutral.svg`，素材通过浅灰明暗、中心叶脉、柔和高光，以及右侧和底部逐渐加深的局部边缘阴影保留轻微真实厚度；阴影不能形成整圈深色描边。每轮动画随机打乱完整多色花瓣色板，不能退化为单色或固定颜色顺序。结算页显示后必须等待嵌套 Container 完成布局，再以小狮子的实际全局中心建立落点；花瓣从屏幕顶部持续向下飘落，位置始终沿现有二次贝塞尔曲线推进，并只用横向缩放模拟柔和翻面，不得让翻面偏移曲线路径。花瓣穿过标题区域时保持清晰可见，只在最后约 `14%` 路程、已经进入小狮子主体附近后逐渐淡出消失，不再把标题或金币图标作为散花落点。花瓣结束前约 `0.34s` 可让小狮子提前开始捧币蓄力，但装饰金币、真实奖励金币和到账音效均须等花瓣完全结束后再触发；最后一枚花瓣结束后不得增加人工停顿。金币到账使用低频、短尾音的阻尼落槽声，禁止高频双金属长鸣。最后一枚金币到达后立即启动数字滚筒，滚轮不再播放固定时长的连续棘轮录音，而是由数字动画在每个真实可视步结束时发出一次短促、柔和的系统选择器式刻度音，刻度间隔跟随 Tween 逐步减速；最终一格由独立的低频锁定音收尾，不与刻度音叠加。视觉阶段按“撒花（尾段允许角色蓄力）→ 金币释放与飞入 → 数字滚动”衔接，音乐本身在撒花后主动降低编配密度，为金币音效留出空间。页面提前关闭时统一取消尚未开始的阶段、正在运行的 Tween 和音乐。所有音频素材由 `tools/generate_audio_sfx.py` 使用固定种子确定性生成，不依赖外部版权资源。
- `result_cheerful.wav` 使用较轻的暖色铺底、短尾卡林巴主旋律、低音量大三度回应和强弱交替的低木质节拍形成轻快弹跳感；不叠加沙锤、刷奏、擦奏或带通噪声打击层，花瓣阶段不得出现周期性的高频摩擦声。
...
| 启动 Splash | 拼块成盘、规则汇聚、狮子戴冠，正常 `6.00s` / 低动效 `2.10s` | `scripts/overlays/splash_overlay.gd`、`assets/ui/splash/*.svg` | 冷启动播放一次；16 张纯路径 SVG，加载未完成时停在最终稳定帧；结束后才释放首页或教程路由 |
...
- Godot 导出的 Xcode 工程必须先运行 `tools/prepare_ios_export.sh` 再构建；该步骤用版本化的 color king storyboard 名称替换模板默认名称，防止 SpringBoard 继续显示旧 Godot 启动快照。
...
1. 运行核心冒烟测试。
2. 运行新手教程冒烟测试。
3. 手动验证首页“开始关卡 / 拼块玩法 / 新人流程”三个主要入口；第 29 关及以前拼块入口应显示第 30 关解锁并弹窗拦截，第 30 关起应与 6x6 普通关卡同步开放并进入拼接阶段，返回首页后主线现场与资源不变。
...
11. 如果修改了 `data/levels.json` 或 `data/composite_levels.json`，必须重新运行二进制目录构建脚本，并执行 `tests/loading_smoke_test.gd`；启动时只允许加载普通关卡目录、拼块 manifest 和当前分发 size，不得恢复全量解析 6000 条拼块 JSON。
12. 如果修改应用图标，必须验证完整合成图背景不透明、自适应前景和单色层保留透明通道，并在圆形与圆角方形蒙版下确认皇冠、狮子脸和伸手动作仍可辨认；Debug 与 Release 导出预设不得回退为把完整方形图同时作为前景层。
13. 如果修改了存档结构，必须验证旧存档兼容。
14. 手动确认首页、关卡页、弹窗和关卡编辑器中的中文均正常显示，不出现缺字方框。
15. 至少完成 1 个 Simple 单/双区域、1 个 Medium 双区域和 1 个 Hard 三区域的 6x6 复合关卡，验证首次动画、Help 重播、顶部单行待放置区左右滚动、3-4 个块可见、颜色发光边框、自动排序、放置撤回、直找、提示浮窗、唯一解转换和中途恢复。
...
5. UI 与交互任务同时按 [`docs/GAME_UI_SKILL_INTEGRATION.md`](GAME_UI_SKILL_INTEGRATION.md) 检查响应式布局、动画生命周期和真机性能验证。
6. 运行自动测试。
7. 按影响范围做手动回归。
8. 提交代码。
