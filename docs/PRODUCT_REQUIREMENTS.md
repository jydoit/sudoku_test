...
- 用户拿起方块后，方块缩放到棋盘格尺寸并保持立体状态；只要完整位于施工区且不与其它已放方块重叠，就显示合法占格预览并允许吸附；越界或重叠时显示错误预览并回弹。
...
- 玩家可以从棋盘重新拿起自己已经放置的方块，并拖回底部托盘；进入托盘回收热区后，系统按横滑内容从头到尾选择第一个空槽，自动滚动并将该槽对焦，方块以半透明缩略图预览。用户在热区内释放后，方块归属该槽并保持原始方向；移出热区则取消本次回收预览。
...
- 离线拼块数据生成器位于 `tools/generate_composite_levels.py`，直接读取输入 JSON 的布局信息，完全在 Python 内执行区域选择、线索筛选、模板拆块、动态 MRV Exact Cover、皇冠唯一解求解和放置顺序生成，不启动 Godot 或调用 GDScript；当前处于代码 review 和抽样阶段，尚未替换正式运行时，也不提交全量生成数据。线索点从非割点安全边界格中按 `max(行异色格数, 列异色格数)` 取最高分，同分时只按固化 seed 选择。离线唯一性只统计同时满足“每块一次、无重叠、填满施工区、方块颜色逐格匹配原始 `regionId`”的完整方案；同颜色、同形状、同方向的视觉相同块只交换内部编号时视为同一方案。每个颜色区域拆分后先独立证明区域铺法唯一，再进入整局搜索；整局搜索使用占格位掩码、剩余等价块组和候选下界组成的记忆化状态，结果截断为 `0/1/2+`。切分期按候选落点数量、重复形状和强制剥离链质量排序，并允许依据第二解对歧义块做同色连通边界局部修正。只有搜索空间耗尽、合法放置方案唯一、恢复布局逐格等于输入且皇冠答案唯一时才允许输出，具体流程见 `docs/COMPOSITE_OFFLINE_GENERATION.md`。
- Hard 默认选择 3 个施工区域；现有 2000 个支持关卡中有 384 个原棋盘只有 2 个区域达到至少 3 格，离线生成时允许显式降级为 2 区域并写入 `difficultyFallback=hard_two_regions`，仍保持 Hard 的 `0.8` 块数系数。该降级当前不影响在线运行时规则。
...
- `scripts/main.gd` 管理 `assembly / transition / crown` 阶段、放置历史、死局结果页、金币自动撤回复活、工具隐藏、Help 默认页、首次演示、最终运行时关卡替换及存档。
...
- 托盘可以横向滑动，超出首屏的全部方块均可访问；手机触摸、按住鼠标拖动、Mac 触控板平移和鼠标滚轮都能驱动横向滚动。手势超过阈值后锁定为横滑或拿块，略微斜向的横滑不能误判为拿取方块。
- 所有位于施工区且不重叠的空间位置均可吸附，不能再由有限预验证布局提前屏蔽；方块可从棋盘拿回托盘，放置期间始终保持立体表现。回收时必须选择头部方向第一个空槽、自动滚动对焦并显示槽内预览，只有在回收热区内释放才修改槽位占用。
