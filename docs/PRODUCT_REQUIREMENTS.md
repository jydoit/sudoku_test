...
- 正式关卡按展示关卡序号分配红心：第 1-10 关 3 个，第 11-30 关 2 个，第 31 关起 1 个；红心为 0 时，本关失败，先停顿 1.5 秒展示最后一个错误红 X，再显示关卡失败页。
...
- 棋盘不能只依赖颜色传达区域信息；棋盘底色为白色，棋盘最外缘不使用黑色边框，不同颜色区域之间不画深色线，统一保留白色格间距、独立圆角和稍明显的底纹，让每个格子保持独立圆角矩形，帮助色弱或对颜色不敏感的用户区分区域轮廓；区域底纹不得采用交叉线或近似 X 的图形，以免与玩家标记混淆。
...
- 首页：主视觉、开始关卡、新人引导入口。
- 新手引导教程：首次进入第一关前，用单张 5x5 教程棋盘图串联双击找小狮子、滑动排除相邻格、点击绿色引导格完成同行同列排除、提示线索和通关目标。
- 关卡页：棋盘、规则提示、选关、提示。
...
- 正式关卡进入或重试时按展示关卡序号重置红心：第 1-10 关 3 个、第 11-30 关 2 个、第 31 关起 1 个；错误小狮子尝试扣 1 个红心，红心为 0 时先锁定棋盘并停顿 1.5 秒展示最后一个错误红 X，再弹出关卡失败页。
- 小狮子直找按钮与提示按钮同一行显示，采用“图标 ×次数”的紧凑横排结构，不显示道具名称；每个用户初始各 3 次，点击后直接把一个未找到的标准答案格标记为锁定小狮子或展示提示范围。免费次数用完后按钮显示 `×0`，点击只弹出购买金币 / 观看激励广告获取积分入口，不直接扣金币使用。
- 新手引导复用同一排小狮子直找和提示按钮，但不显示无限次数：非当前教学阶段的道具计数为空；当引导用户点击提示或小狮子按钮时显示 `×1`；点击完成后显示 `×0`，且不消耗正式关卡免费次数、金币或红心。
...
- 棋盘支持鼠标点击和触屏点击。
- 移动端棋盘输入以触屏事件为准，不能把 Android 派发的触屏事件和兼容鼠标事件重复处理成两次点击。
- 双击找小狮子由项目自定义判定：必须是同一格的两次完整按下/抬起，间隔在安全窗口内，移动距离小，且没有进入滑动状态；不直接依赖 Godot 自带 `double_tap` 或 `double_click` 标记。
- 有冲突时冲突格变红。
- 正式关卡使用提示时，全部提示相关格和已有 X/小狮子/错误 X 保持正常亮度，不加高亮框、不闪烁；其它空格覆盖半透明白灰浮层，并禁止点击。新手教程的相邻排除和同行同列排除阶段不使用蒙层：棋盘全部保持原亮度，本阶段全部合法目标格同时开放并显示轻量金色细边。相邻排除可用小手滑动演示连续标记 X；同行同列排除必须用小手点击绿色引导格演示。真机上从绿色目标格起手的轻微滑动按一次点击容错处理，但不得沿路径批量完成同行同列阶段。
...
- 棋盘格子之间的缝隙统一使用白色底板填充：每个格子的圆角留白也归入间隔层，同色相邻格和不同色相邻格之间都不画深色线，只保留白色格间距，避免格子合并成大色块；棋盘最外缘也不画深色外框，主关卡和教程保持一致。
...
- `GameBoard` 自行维护触控防抖：触屏后短时间内忽略兼容鼠标事件；单击立即触发普通 X 反馈；第二次点击只有同格、同位置附近、间隔符合 `DOUBLE_TAP_MIN_MS` 到 `DOUBLE_TAP_MAX_MS` 时才升级为双击小狮子尝试；拖动开始后清除双击候选并只发滑动信号。
...
- 棋盘使用白色底板、独立圆角格、明亮 10 色色板和更明显的底纹，让颜色区域与单个格子都能被识别。
- 棋盘最外缘不画深色外框；格子之间保留白色间距，同色区域内部和不同色交界都不画深色分割线。
...
- 点击小狮子按钮会直接找到一个锁定小狮子并优先消耗 1 次免费次数；免费次数为 0 后显示 `×0`，点击弹出购买金币 / 观看激励广告获取积分入口，不直接放置小狮子。
...
- 提示消耗免费次数；免费次数用完后按钮显示 `×0`，点击弹出购买金币 / 观看激励广告获取积分入口。
- 免费提示初始为 3 次。
- 免费次数用完后，本次点击不直接消耗金币、不展示负数或 `-价格`，也不立即给出提示。
...
- 免费逻辑提示和免费小狮子直找次数耗尽后，按钮显示 `×0`，点击时弹出获取入口；当前版本不在点击时直接消耗金币执行道具。
...
- 免费次数耗尽后只弹出“购买金币 / 观看激励广告 / 稍后再说”选择，不自动播放广告、不打断正常解题。
...
- 道具按钮最低显示 `×0`，不得显示负数或 `-价格`。
- 免费次数耗尽后点击道具不修改道具次数或棋盘状态，并展示自愿获取金币 / 激励广告入口。
- 失败页点击“重新挑战”会立即重置当前关卡和红心；点击“返回首页”视为放弃本次失败局，回到首页并准备一盘新的当前关卡。
...
- 首页点击“开始关卡”时，如果当前正式关卡已经失败或存档恢复为失败状态，会重新开始该关，恢复红心并清空本次失败局的错误红 X，不再重新显示失败页。
...
- 已失败关卡从首页继续时应重新开始当前关卡，不能进入隐藏弹窗但棋盘锁住的状态，也不能再次显示旧失败页阻断进入。
...
- 教程必须包含双击找小狮子、滑动批量排除相邻 X、小狮子相邻排除、点击绿色引导格完成同行同列排除、提示线索、小狮子直找和完整找出全部小狮子。
...
3. 相邻格排除完成后，进入同行同列排除阶段，文案显示“每行、每列都只能有一个小狮子。这个小狮子所在的行和列，其他格都可以标记 X。”；小手必须指向绿色引导格做单次点击演示，不做横向滑动。玩家需要点击绿色引导格完成该步骤；如果手指从绿色目标格起手后轻微滑动，也按一次点击容错处理。
...
- 教程由 `TUTORIAL_LEVELS` 的 `single_map` 配置和教程状态机驱动；双击找小狮子等单目标步骤可使用单格聚焦，相邻排除和同行同列排除阶段取消棋盘蒙层、一次开放当前规则组全部合法格。相邻排除阶段配合小手滑动动画；同行同列排除阶段小手只做点击绿色引导格的演示，并允许从绿色目标格起手的轻微滑动按一次点击容错，但不允许拖动路径批量完成；提示与小狮子直找按钮使用按钮点击引导，最后进入开始挑战入口。
- 教程与正式关卡共用顶部、进度和底部道具组件；提示和小狮子直找只在对应教学阶段启用，计数只显示当前教学道具的 `×1`，使用后显示 `×0`，其它阶段为空。
- 正式关卡页隐藏教程按钮。

回归测试重点：

- 新存档首次启动必须直接进入单张 5x5 新手教程棋盘，而不是停留在首页或直接进入第 1 关。
- 教程必须完整展示双击找小狮子、滑动排除相邻格、点击绿色引导格完成同行同列排除、提示线索、小狮子直找和核心规则。
- 教程保持正式关卡两道具布局；提示和小狮子直找按教程阶段依次启用。
- 教程道具计数不得显示 `×∞` 或正式关卡真实次数；只在引导点击该道具时显示 `×1`，点击后显示 `×0`。
...
| 逻辑提示获取基准 | 1 × 本关奖励基数 | `TOOL_HINT` | 免费提示耗尽后用于获取入口估算，不在按钮上显示负数 |
| 小狮子直找获取基准 | 3 × 本关奖励基数 | `TOOL_CROWN_FIND` | 免费直找耗尽后用于获取入口估算，不在按钮上显示负数 |
| 首次获取折扣 | 20% 基数，3 次兑换内回归 | `INACTIVE_TOOL_DISCOUNT_RATE` | 最近 3 次通关未兑换时生效 |
...
| BOARD-008 | 红心降至 0 | 棋盘锁定，最后一个错误红 X 保持可见 1.5 秒后进入失败页 | A+V+D | P0 |
| BOARD-009 | 制造同行、同列、同区域和相邻冲突 | 开启即时纠错时准确标出冲突，通关判定仍拒绝冲突 | A | P0 |
| BOARD-010 | Android/触屏单击一次空格 | 只触发一次普通 X，不触发小狮子尝试 | A+M+D | P0 |
| BOARD-011 | 同一格两次完整点击，间隔在安全窗口内且移动距离很小 | 第一次点击立即给出普通 X 反馈，第二次点击升级为一次小狮子尝试 | A+M+D | P0 |
| BOARD-012 | 两次点击间隔过短、过长、跨格或移动距离过大 | 不触发双击，按各自合法单击处理 | A+M+D | P0 |
| BOARD-013 | 同一次触屏操作后收到 Android 兼容鼠标事件 | 只处理触屏输入，不重复触发单击或双击 | A+D | P0 |
| BOARD-014 | 从格子开始滑动到其它格子 | 只进入滑动批量 X 逻辑，不触发单击或双击 | A+M+D | P0 |
| BOARD-015 | 单击/双击已锁定小狮子、提示小狮子、开局小狮子或错误红 X | 锁定状态完全不变，不触发新操作 | A+M+D | P0 |
...
| TOOL-002 | 免费次数耗尽后使用道具 | 按钮显示 `×0`，点击弹出购买金币 / 观看激励广告获取积分入口，不直接执行道具 | A+M | P0 |
| TOOL-003 | 免费次数耗尽弹窗 | 弹窗展示获取入口，不产生虚假到账 | A+V | P1 |
...
| TUTOR-003 | 教程使用提示和小狮子直找 | 非教学阶段道具计数为空；引导点击时显示 `×1`；点击后显示 `×0`；直找放置锁定小狮子但不消耗正式次数或金币 | A+V | P0 |
| TUTOR-004 | 完成或确认跳过教程 | 保存完成状态并进入正式第 1 关 | A+M | P0 |
| RESULT-001 | 完成正式关卡 | 显示蓝色成功页、胜利小狮子、下一关和主菜单 | A+V+D | P0 |
| RESULT-002 | 红心耗尽 | 最后一个错误红 X 先停顿展示 1.5 秒，再显示蓝色失败页、失败小狮子、重新挑战和返回首页，无金币复活 | A+V+D | P0 |
| SAVE-001 | 标记棋盘、修改资源后重启 | 当前关卡、棋盘、金币、次数和完成记录正确恢复；临时提示蒙层、拖动状态和弹窗不残留 | A+M+D | P0 |
| SAVE-002 | 使用上一存档版本启动 | 兼容迁移或安全回退，不丢失可保留进度 | A+M | P0 |
| SAVE-003 | 红心归零失败后返回首页或重启并点击开始关卡 | 重新开始当前关卡，红心恢复，错误红 X 清空，不再次显示旧失败页 | A+M+D | P0 |
