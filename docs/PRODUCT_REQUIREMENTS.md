...
- 正常模式总时长控制在 `3.90-4.30s`，减少动画模式控制在 `1.80-2.10s`；资源已经准备好时仍完整展示各阶段，资源加载较慢时停留在最后一个稳定画面，不循环抖动或制造假的百分比进度。
...
| 时间 | 画面动作 | 声音与节奏 |
| --- | --- | --- |
| `0.00-0.60s` | 蓝天渐变背景淡入；不完整的 6x6 装饰棋盘和三个待放异形拼块出现，第一块被拿起、沿固定路径进入缺口并吸附。 | `0.60s` 播放中等尺寸木质放置音。 |
| `0.60-1.20s` | 第二块被拿起、移动并吸附；棋盘、镜头与其它拼块保持同一注册位置，拼块不旋转。 | `1.20s` 播放第二次木质放置音。 |
| `1.20-2.20s` | 最后一块从棋盘下方升起，沿轻微曲线路径移动，在缺口上方悬停后向下落位。 | `2.20s` 使用稍完整的大块放置音。 |
| `2.20-2.70s` | 完整棋盘只做一次 `1.00 -> 1.025 -> 0.992 -> 1.00` 收束回弹，拼块厚度和阴影退出，压平为普通皇冠棋盘。 | `2.45s` 播放一次拼块完成音，不叠加屏幕震动。 |
| `2.70-3.15s` | 同一棋盘依次出现行、列和颜色区域高光，三者汇聚到装饰皇冠格；标准小狮子从棋盘后方上升，`color king` 标题同步淡入。 | `3.15s` 使用现有皇冠揭示短音。 |
| `3.15-3.75s` | 棋盘、皇冠、小狮子和标题完全静止；资源未就绪时停在此稳定画面。 | 保持静音，不循环。 |
| `3.75-4.10s` | 目标页面已准备好时，整个 Splash 统一淡出并释放输入。 | 不新增离场音。 |
...
减少动画模式使用 `1.65s` 的淡入和三张稳定关键帧（未完成拼块、完整平面棋盘、狮子戴冠），再用 `0.35s` 淡出；取消逐块飞行、完成回弹和高光闪动。普通时间轴在完整棋盘出现后允许点击、触摸或键盘跳过，跳过与自然结束都只能释放一次启动路由。

Splash 验收标准：

- 新用户启动时，Splash 结束后进入单张 5x5 新手教程；已有存档用户进入原本应进入的首页/关卡/恢复流程，动画不改变路由判断。
- 540x960 竖屏下标题、棋盘示意图、小狮子和淡出过渡均不裁切；窄屏下无文字溢出或安全区遮挡。
- 真实资源已就绪时正常模式总时长约 `4.10s`、减少动画模式约 `2.00s`；资源未就绪时不显示空白页面、不循环跳动，准备完成后从稳定画面自然退出。
...
| 启动 Splash | 拼块成盘、规则汇聚、狮子戴冠，正常 `4.10s` / 低动效 `2.00s` | `scripts/overlays/splash_overlay.gd`、`assets/ui/splash/*.svg` | 冷启动播放一次；16 张纯路径 SVG，加载未完成时停在最终稳定帧；结束后才释放首页或教程路由 |
