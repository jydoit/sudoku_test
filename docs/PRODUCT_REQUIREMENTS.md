...
部分特殊关卡在找皇冠前增加短时的颜色块拼接阶段：棋盘大部分颜色区域已经锁定，玩家只补完 1-2 个被挖空的完整颜色区域。拼接完成后，棋盘恢复为标准二维颜色区域，再进入相同的找皇冠核心玩法；拼块阶段不改变皇冠阶段的四条核心规则。
...
- 本地存档。
- Godot 冒烟测试和新手教程冒烟测试。
- 6x6 及以上里程碑挑战关的复合拼块流程：随机切出 1-2 个颜色区域、立体方块托盘、放置撤回、首次演示、Help、二维转换、动态唯一皇冠答案与存档恢复。
...
- “拼块玩法”按钮在完成新人流程后可用，直接进入独立的 6x6 复合拼块挑战；进入前保存主线关卡、资源与未完成棋盘，返回首页时完整恢复。独立体验不增加主线关卡号、通关记录、金币或关卡导演统计。
...
- 皇冠直找按钮与清除、提示按钮同一行显示；每个用户初始 3 次，点击后直接把一个未找到的标准答案格标记为锁定皇冠，免费次数用完后显示金币价格。
...
- 棋盘格子之间的缝隙使用底层填充：每个格子的圆角留白也归入间隔层，同色相邻格之间使用该色的加深色；不同色块边界和棋盘最外缘统一使用预合成后的深色外框色，外框以四条整数像素边带绘制，保证主关卡和教程中的宽度、颜色一致，也减少纯白分割带来的简陋感。
...
- 棋盘不会从完全空白开始。开局锁定大部分完整颜色区域，只挖空 1 个或 2 个完整颜色区域。
- 只挖空 1 个颜色区域时，该区域的总占格固定，玩法目标是用同色方块复原完整区域，完成后继续使用原关卡皇冠答案。
- 挖空 2 个相邻颜色区域时，两者合并为一个不显示原内部边界的施工区；两种颜色方块在施工区内可形成多个经过验证的连续边界，完成后根据最终布局匹配并锁定对应的唯一皇冠答案。
- 复合关卡属于普通找皇冠流程的前置阶段，不独立结算奖励，不额外消耗红心、金币或道具次数。

关卡阶段：

1. `assembly_intro`：首次体验时播放拼块流程动画；已经看过时直接进入拼接。
2. `assembly`：显示锁定的立体颜色区域、空白施工区和底部方块托盘。
...
```text
顶部：返回首页 / 金币 / 红心 / Help / 设置 / 选关
标题：第 X 关 + “先拼好颜色区域”阶段标识
中部：正方形 6x6-9x9 棋盘
底部：深色横向滑动方块托盘（替换三个道具）
```

- 拼接阶段保留顶部导航与关卡标题，隐藏皇冠进度，避免在答案尚未锁定前传达错误进度。
- 拼接阶段完全隐藏清除、皇冠直找和 Hint，而不是显示禁用态；Help 和设置仍可用。
- 托盘位于底部安全区内，使用单行横向滚动；方块超过可视宽度时，用户左右滑动查看更多方块。
- 拼接结束后页面骨架不换页：阶段标识淡出、皇冠进度出现、深色托盘向下退出、清除/皇冠直找/Hint 从原位置恢复。

棋盘与方块视觉：

- 已锁定颜色区域使用原“明亮经典”颜色，但通过顶部高光、下/右侧面和柔和投影表现为立体方块；锁定区域不可点击、拖动或旋转。
- 施工区显示为低饱和中性凹槽，保留格子坐标和施工区外轮廓，不提前显示两个待拼颜色区域之间的正确边界。
- 托盘背景使用深色系，与暖白关卡背景形成明确层级；托盘中的方块保持所属区域颜色，并使用与棋盘一致的立体材质。
- 用户拿起方块后，方块缩放到棋盘格尺寸并保持立体状态；合法落点显示完整占格预览，越界、重叠或无合法后续完成路径时显示错误预览并回弹。
- 方块成功放到棋盘后仍保持立体状态；只有所有施工格完成且布局通过校验后，才在过场动画中统一压平为标准二维区域。
- 颜色不能成为唯一状态信息。合法/非法落点同时使用轮廓、占格预览和回弹反馈，锁定区与可拼区通过高度、凹槽和交互反馈区分。

拼接交互：

- 托盘中的横向手势用于滚动；从方块明显向上拖动时拿起方块，避免横向浏览和拿取冲突。
...
- 玩家可以从棋盘重新拿起自己已经放置的方块，并拖回底部托盘；回收后恢复该方块的托盘槽位和原始方向。
- 已锁定区域始终不能被拿起；拼接阶段不响应普通 X、滑动 X、双击皇冠或任何道具行为。
- 每次成功放置、移动和回收方块后立即保存；退出再进入时恢复相同托盘顺序、方块位置和阶段。
...
- 放置系统只接受仍能通向至少一个预验证最终布局的中间状态；不能等到最后一块才把无解责任交给用户。
...
- 先从已经满足唯一解、连续区域和 `no_guess` 的 6x6-9x9 基础关卡选择 1-2 个待拼完整区域；双区域模式优先选择相邻且合并后连续的区域。
- 单区域模式建议生成 2-3 个同色方块；双区域模式建议总计生成 4-6 个方块。每块至少包含 2 格，禁止断开、内部孔洞和只含 1 格的碎片。
- 基础形状池包括矩形、横条、竖条、L 形、T 形、S/Z 阶梯形以及无孔的连续异构多格体；通过旋转产生方向变体并按规范化坐标去重。
- 简单难度提高矩形和短条权重；普通难度增加 L 形与阶梯形；困难及挑战难度增加非对称异构体，但仍不得依赖盲试才能完成。
- 切分器使用可持久化随机种子。每次从区域中提取一个候选块后，必须确认该块连续、剩余部分仍可继续分割且最终所有块可以完整覆盖施工区。
- 对生成的方块集合枚举或搜索完整铺法，删除区域断开、无皇冠解、多皇冠解、无法无猜推进或难度越界的结果。
- 单区域模式至少保留 1 个合法完整铺法；双区域模式目标保留 2-6 个不同颜色边界的合法铺法，避免既完全无选择也过度开放。
- 运行时优先匹配预验证布局签名及其皇冠答案；不允许为了得到答案而偷偷改动玩家已经拼出的颜色边界。

首次流程动画与 Help：

- 用户账号第一次进入任意复合拼块关卡时自动播放一次流程动画，后续复合关卡不再强制播放。
- 动画依次展示：锁定区域不可移动、施工区目标、横向滑动托盘、向上拖出一个方块、合法吸附、把已放方块拖回托盘、完成后立体棋盘转换为二维皇冠棋盘。
...
- Help 文案至少覆盖：彩色方块只能放入凹槽、左右滑动查看更多、方块需完整对齐、已放方块可拖回托盘、同色必须连续、拼完后进入找皇冠。

过场动画：

- 最后一块合法吸附后立即锁定棋盘和托盘输入，避免完成瞬间又被拿走。
- 所有立体方块先进行一次短促落位确认，再同步降低侧面高度和投影，合并同色内部拼块缝隙，只保留最终颜色区域边界。
- 深色托盘向下滑出，页面背景恢复标准关卡视觉；阶段标识淡出，皇冠进度和三个道具淡入。
...
- 存档新增复合玩法阶段、随机种子、托盘顺序、各方块放置坐标、最终布局签名、锁定后的 `regions` 与 `solution`，并保存 `compositeTutorialSeen`。

当前实现：

- `scripts/composite_level.gd` 根据基础关卡和确定性种子选择 1-2 个区域，使用矩形、条形、L/T/阶梯和异构模板做连通切分，枚举能够完整覆盖施工区的摆法，并只保留唯一皇冠解布局。
- 新关卡优先在多个确定性种子中选择至少包含 2 个合法最终布局的切分，找不到时回退到至少 1 个合法布局；保存恢复始终复用已经写入的种子与布局，不重新抽取。
- `scripts/assembly_view.gd` 绘制立体锁定区域、凹槽、深色横向托盘、拖动预览和压平过场，并处理鼠标与单指触控的滚动/拖放仲裁。
- `scripts/level_director.gd` 在 6x6 及以上里程碑挑战日程中写入 `assemblyEnabled`，拼块期间不安排开局提示皇冠。
- `scripts/main.gd` 管理 `assembly / transition / crown` 阶段、工具隐藏、Help 默认页、首次演示、最终运行时关卡替换及存档。
- `tests/composite_smoke_test.gd` 覆盖随机切分、方块连通、多布局、拼块 UI、道具隔离、Hint、转换和两阶段重启恢复。

回归测试重点：

- 复合关卡尺寸不小于 6x6，初始棋盘不会完全空白。
- 锁定区域不可交互，施工区之外不能放置方块。
- 拼接阶段不显示也不能触发清除、皇冠直找和 Hint，不产生任何道具或金币消耗。
- 托盘可以横向滑动，拿取方块不会误触为滚动；超出首屏的全部方块均可访问。
- 方块可合法吸附，可从棋盘拿回托盘；放置期间始终保持立体表现。
- 每个随机方块自身连续、至少 2 格、无孔；完整区域不会被切成断开的最终颜色岛。
- 无解或多解的完整铺法不能进入皇冠阶段；合法完成后最终答案唯一且可无猜推进。
- 首次复合关卡自动播放流程动画，之后不重复强制播放；Help 可随时重播。
- 过场结束后棋盘恢复标准二维颜色模式，皇冠进度与三个道具恢复并可正常使用。
...
- 通关结果页使用整屏奖励结构：蓝色背景、顶部完成文案、中部皇冠展位和底部按钮。
...
- 复合拼块玩法保存当前阶段、切分种子、托盘顺序、方块放置、最终布局与锁定答案，并保存是否已经看过首次拼块流程动画。
...
- 当前保存字段包括 `currentLevelIndex`、`currentLevelId`、`playerLevelNumber`、`activeSchedule`、`directorProgress`、`economyProgress`、`runStartedUnix`、`runMoveCount`、`runHintCount`、`runCoinExchangeCount`、`cellStates`、`isCompleted`、`isFailed`、`coinCount`、`heartCount`、`hintCount`、`completedLevels`、`immediateErrors`、`tutorialCompleted`、`tutorialStarted`、`tutorialStepIndex`、`formalProgressSnapshot`、`homeCompositeEntryActive`、`homeCompositeProgressSnapshot`、`compositeState`、`compositeTutorialSeen`。
- `directorProgress` 保存最近通关记录、size/difficulty 统计、下一关开启和次留补记状态。
- 首页点击“开始关卡”时，如果当前恢复的正式关卡已经完成，会自动推进并加载下一关，避免停留在已完成且不可操作的棋盘。
- `SAVE_VERSION` 当前为 10；`formalProgressSnapshot` 在重看教程时持久保存正式关卡索引、调度、通关记录、经济资源、棋盘状态、红心、本局统计和复合拼块现场，成功恢复后清除。
- `homeCompositeProgressSnapshot` 与 `homeCompositeEntryActive` 用于隔离首页拼块入口：即使在独立拼块体验中退出应用，下次启动回到首页时也先恢复进入前的主线现场。
- `compositeState` 保存关卡 ID、阶段、切分种子、已放方块、最终布局签名、`regions` 与 `solution`；版本 8 及更早的旧存档没有该字段时按当前关卡日程重新开始拼块或直接加载普通皇冠关卡。
...
| 首页拼块入口 | 完成新人流程后常驻 | `_start_home_composite_flow()` | 独立 6x6 挑战；退出后恢复主线现场，不结算主线奖励 |
| 复合拼块空缺区域 | 1-2 个完整颜色区域 | `CompositeLevel.build()` | 其余颜色区域开局锁定且不可交互 |
| 复合拼块数量 | 单区域 2-3 块；双区域合计 4-6 块 | `scripts/composite_level.gd` | 每块至少 2 格、连续且无孔 |
| 复合拼块旋转 | 生成时允许 0/90/180/270 度 | `scripts/composite_level.gd` | 第一版玩家不主动旋转或镜像 |
| 拼块转换动画 | 0.88 秒，规范允许 0.8-1.4 秒 | `AssemblyView.play_flatten_transition()` | 输入锁定，托盘退出并恢复皇冠 UI |
| 默认关卡数 | 2470 | `data/levels.json` | MVP 内置关卡 |
...
13. 至少完成 1 个单区域和 1 个双区域 6x6 复合关卡，验证首次动画、Help 重播、托盘滑动、放置撤回、唯一解转换和中途恢复。
14. 更新本文档对应模块。
