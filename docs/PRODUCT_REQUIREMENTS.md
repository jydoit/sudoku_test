...
- 当前不显示广告位。
- 应用启动 Splash 动画尚未接入；候选脚本和验收规则见 5.1，默认推荐方案 A。
...
当前状态：设计候选，尚未接入运行时。确认方案后再创建 `scripts/overlays/splash_overlay.gd` 及对应场景，不在 Splash 中读取或修改真实棋盘、皇冠答案、红心和金币。

统一约束：

- 仅在应用冷启动或重新启动应用时播放；首页、关卡、结果页之间的普通路由切换不重复播放。
- 默认总时长控制在 `0.85-1.40s`；资源已经准备好时仍保留至少 `0.85s` 的品牌展示，资源加载较慢时停留在最后一个稳定画面，不循环抖动或制造假的百分比进度。
...
实现建议：使用一个 `CanvasLayer` 下的 `Control` 作为 Splash 根节点，`AnimationPlayer` 保存固定时间轴和信号点，`Tween` 只处理一次性的淡入、回弹和离场；不使用 `AnimationTree`。启动期间并行准备存档、关卡目录和本地化资源，Splash 通过 `ready_to_enter` 信号等待真实启动状态完成。建议保留 `boot_started`、`splash_ready`、`splash_finished` 和 `splash_skipped` 四个语义事件，避免 `scripts/main.gd` 直接操作 Splash 内部节点。

#### 方案 A：小狮子点亮棋盘（默认推荐）

定位：最适合作为默认冷启动动画。它同时表达颜色区域、棋盘边界、皇冠狮子和“找到皇冠”的核心目标，信息密度适中，首次启动不会误解为另一种玩法。

动画脚本：

| 时间 | 画面动作 | 声音与节奏 |
| --- | --- | --- |
| `0.00-0.16s` | 蓝色背景淡入，`color king` 标题从 `96%` 轻微放大到 `100%`；中央留出棋盘构图空间。 | 很轻的暖色上行铺底。 |
| `0.16-0.48s` | 5x5 抽象棋盘由中心向外铺开，明亮色块分三批出现；黑色外框和区域边界随后补齐，边界只画一次。 | 三次柔和纸面/木质点击，间隔约 `80ms`。 |
| `0.44-0.70s` | 戴皇冠的小狮子从上方进入中央装饰格，落地时做一次 `1.04 -> 0.96 -> 1.00` 的轻回弹；该格不是实际答案格。 | 一次短促但不尖锐的金属亮音。 |
| `0.70-0.94s` | 一条行线、一条列线和一个颜色区域边界依次亮起后熄灭，提示玩家“行、列、颜色区域”三类观察维度；不出现解释文字。 | 三个低音量确认音，最后接短尾音。 |
| `0.94-1.18s` | 棋盘和狮子保持稳定 `0.12s`，标题回到正常透明度；整体向首页或教程方向交叉淡出。 | 音效尾音与页面进入同步，不叠加屏幕震动。 |

Godot 脚本建议：`AnimationPlayer` clip 命名为 `splash_brand_reveal`，棋盘示意图由独立 `SplashBoardIllustration` 绘制，使用现有色板和狮子贴图；加载完成后通过 `animation_finished` 或 `splash_ready` 选择 `finish` / `hold` 分支。方案 A 的反馈强度属于小型 UI 反馈，不使用 hit-stop 和 screen shake。

优点：品牌识别最快，和普通皇冠关卡、教程入口都兼容。注意点：棋盘必须是装饰副本，不能复用真实关卡状态，避免启动动画改变答案或触发提示逻辑。

#### 方案 B：彩色拼块合板

定位：突出复合拼块是产品差异点，适合作为拼块玩法首次解锁、拼块入口专属启动过场或后续 A/B 版本，不建议作为所有用户每次冷启动的默认动画。

动画脚本：

| 时间 | 画面动作 | 声音与节奏 |
| --- | --- | --- |
| `0.00-0.18s` | 蓝色背景出现，四个带侧面的彩色方块从屏幕四边滑入，落点形成松散的方形轮廓。 | 低频木质拖动声。 |
| `0.18-0.56s` | 6-8 个抽象拼块依次吸附到轮廓内；每次吸附只做短促位置回弹，不旋转、不遮挡标题。 | 每次吸附一个短点击，最后一次音量略高。 |
| `0.56-0.78s` | 拼块整体压平为二维颜色区域，黑色区域边界从接缝处向外展开。 | 一次柔和“合板”低音。 |
| `0.78-1.04s` | 小狮子从拼块中心升起，皇冠上的高光扫过一次；色块停止运动。 | 短金属亮音。 |
| `1.04-1.30s` | 画面淡出到目标页面；如果实际加载未完成，停留在已经压平的稳定棋盘上。 | 尾音保持短，不循环。 |

Godot 脚本建议：使用 `AnimationPlayer` 控制固定的入场顺序，`Tween` 控制每个拼块的吸附和回弹；拼块只使用装饰数据，不调用 `CompositeLevel`、不改变托盘顺序、不写入 `compositeState`。

优点：能一眼体现“拼接颜色区域再找皇冠”的独特流程。注意点：对新用户而言，可能把拼块误认为所有关卡的第一步，因此更适合拼块入口或解锁庆祝，不宜默认覆盖普通启动流程。

#### 方案 C：皇冠规则扫描

定位：强化逻辑谜题和“观察后找到皇冠”的感觉，适合已有正式进度的玩家、教程入口或低动效版本。

动画脚本：

| 时间 | 画面动作 | 声音与节奏 |
| --- | --- | --- |
| `0.00-0.14s` | `color king` 标题淡入，背景保持静止，中央出现空白棋盘轮廓。 | 单个很轻的起始音。 |
| `0.14-0.40s` | 棋盘格线从左上向右下绘制；颜色区域先填充，黑色边界最后闭合。 | 连续但低音量的纸面划线声。 |
| `0.40-0.66s` | 一条柔和扫描光沿一行和一列移动，在交叉处短暂停留；不显示真实关卡候选。 | 两个短确认音。 |
| `0.66-0.88s` | 扫描光沿一个颜色区域边界绕行，边界亮度升高后恢复。 | 轻微环绕音，不使用闪屏。 |
| `0.88-1.10s` | 小狮子在扫描交叉附近出现并做一次眨眼/点头，表达“找到皇冠”。 | 单次温和收束音。 |
| `1.10-1.24s` | 整体淡出，进入首页或教程。 | 尾音结束后释放输入。 |

Godot 脚本建议：网格和扫描线由 `Control._draw()` 或独立绘制节点完成，扫描线位移使用 `Tween` 的 ease-out；不要创建真实 `GameBoard`，避免启动期间运行冲突检查、提示蒙层或皇冠答案逻辑。

优点：玩法表达最准确、动效最克制。注意点：品牌记忆点和情绪张力弱于方案 A，首次启动需要配合更清楚的首页主视觉。

推荐决策：默认冷启动先实现方案 A；方案 B 留给拼块入口或拼块解锁庆祝；方案 C 可作为减少动画模式或老玩家启动变体。三套方案均应保持同一时长区间、同一狮子贴图和同一蓝色背景，避免不同入口像不同产品。

Splash 验收标准：

- 新用户启动时，Splash 结束后进入单张 5x5 新手教程；已有存档用户进入原本应进入的首页/关卡/恢复流程，动画不改变路由判断。
- 540x960 竖屏下标题、棋盘示意图、小狮子和淡出过渡均不裁切；窄屏下无文字溢出或安全区遮挡。
- 真实资源已就绪时总时长不超过 `1.40s`；资源未就绪时不显示空白页面、不循环跳动，准备完成后从稳定画面自然退出。
...
- 点击开始关卡进入关卡页。
- 点击新人流程会重新进入新手引导。
- 第 10 关及以前点击拼块玩法会弹出“玩到第 11 关即可解锁”的提示，不保存或替换当前主线现场；第 11 关起点击会进入 6x6 拼接阶段，从关卡顶部或结果页返回首页后，主线关卡、棋盘现场和资源恢复到进入前状态。
...
- 皇冠直找按钮与清除、提示按钮同一行显示；每个用户初始 3 次，点击后直接把一个未找到的标准答案格标记为锁定皇冠。三个按钮统一使用紧凑的“图标 + 简短名称 + 固定高度状态胶囊”结构：清除显示“免费”，直找和提示有次数时显示“免费 ×N”；次数为 0 时不显示 `×0`，状态胶囊直接切换为金币图标和本次价格。付费胶囊必须使用不透明底色：直找沿用金币黄体系，提示使用提示绿体系，不能只依赖透明金币贴图承载价格；金币图标使用 SVG 矢量源，避免小尺寸道具价格出现位图毛刺。工具栏不得挤压棋盘或在 540×960 竖屏下越过底部安全区。
...
- 动态 size 解锁节奏由 `SIZE_UNLOCK_DISPLAY_LEVELS` 统一控制：第 1 关开放 5x5，第 11 关起开放 5x5-6x6，第 80 关起开放 5x5-7x7，第 160 关起采样 6x6-8x8，第 240 关起采样 6x6-9x9。
...
- 三个方向都禁止复用头像、透视 Shader 或控件横纵缩放来冒充姿势变化，支撑阶段双爪必须锁定物理屏幕边缘，离边后才前往角色展位中心。边缘接触帧与居中腾空帧的注册差必须在起跳首帧按方向补偿，并在跳跃前 `28%` 路程内平滑释放，不能让角色刚离边就横向或纵向闪移。小狮子到达展位后，入场层只在落地阶段完成唯一一次位置缓冲，并从边缘展示尺寸连续缩放到展位的 `164px` 尺寸；接触展位后立即统一使用不透明、已注册的手臂外伸交接帧 `lion_king_center_landing.svg` 完成整段落地，禁止把腾空整身图与中央身体做透明混合或形变插帧，以免产生第二只手臂、尾巴重影或闪动。交接帧由 `tools/generate_lion_center_layers.py` 与中央身体、手臂层一起确定性生成。落地结束后的抵达庆祝必须锁定角色中心坐标、展位尺寸和 `0` 旋转，只允许播放独立手臂关键帧，不得再次上下位移、旋转或缩放。
- 中央庆祝统一使用固定身体与独立手臂双层结构：`lion_king_center_body.svg` 始终保持同一纹理，`lion_king_center_arm_00.svg` 至 `12.svg` 使用相同 `400 x 400` 注册画布并在身体后方只绘制左臂和手掌；身体、脸、皇冠、鬃毛、腿和尾巴不得随动作关键帧重绘。手臂源姿势来自 `docs/animation_sources/lion_center_wave_sheet_v2.png`，分层资源由 `tools/generate_lion_center_layers.py` 确定性生成并保持纯路径 SVG。自然挥手按 `13` 帧正放、去除外伸端点重复后 `12` 帧反放组成 `25` 帧单次动作，移动帧约每 `0.04s` 切换一次，抬手端和完全外伸端分别短暂停顿；欢呼、调皮和抛金币也只允许切换手臂关键帧，身体 transform 始终锁定。抛金币必须执行完整的 `0 → 12 → 0` 手臂序列，不能按金币数量抽样跳帧。每个随机动作只播放一次，回到完全静止的待机姿势 `1.5-2.7s` 后再播放下一种，且不能连续重复同一种。旧的整身吐舌、眨眼、鬼脸和全身挥手资源不进入运行包。GOOD 的飞币由入场完成事件触发，不使用固定时长计时器；EXCELLENT 的入场可与撒花同时进行，但金币仍由撒花完成事件放行，两个条件都满足后才开始。
- 结算页只展示真实产生的金币奖励，不展示没有后续用途的“获得皇冠 +1”虚拟奖励；奖励行只保留金币图标和右侧金色大号账户余额，不再显示 `coins / 金币奖励` 文字。关卡顶部余额保持 `24px` 和至少 `62 x 44px` 的紧凑固定布局；结算页余额改为按实际位数自适应：一至六位分别使用 `38/36/34/32/29/26px` 字号以及至少 `42/58/76/94/108/122px` 数字区宽度，七位及以上使用 `23px` 并依据字体实测继续扩宽。结算金币图标使用 `44 x 44px`；金币图标和数字裁切区之间必须插入独立的 `28px` 布局占位节点，不依赖 HBox 主题 separation，数字区左侧另保留 `8px` 内边距，使图标控件右边缘到数字字形起点至少约 `36px`。奖励组保持整体居中，使金币相对向左、数字相对向右展开。滚动动画开始前按 `max(初始余额位数, 目标余额位数)` 一次性锁定本轮字号和数字区宽度，滚动过程中不得改变布局；动画停止后才允许按最终余额位数收紧，例如 `99→100` 全程使用三位规格，`100→99` 停止后才收紧为两位规格。通关奖励可以立即写入钱包和存档，但关卡页顶部必须继续显示奖励前余额，禁止在局内完成庆祝阶段提前展示增加；只有结算页负责可见的到账过程。结算页必须接收钱包交易返回的明确到账前、到账后余额，并在奖励行首次显示前写入到账前余额，首帧不得显示组件默认的 `0`、上一局残留余额或已经到账后的余额；页面淡入和 Excellent 撒花期间都保持该奖励前余额。页面淡入后停顿约 `0.45s`，小狮子配合挥手/抛金币动作，将与奖励数量一致的金币沿交错弧线逐枚抛向奖励行的金币图标。数字宽度更新后必须等待容器完成布局，再以金币图标的实际全局中心建立飞行终点，禁止使用旧布局位置或奖励行边界作为接收焦点；每枚金币落入时只触发目标图标的轻微回弹，余额在全部飞币完成前保持初始值。最后一枚金币完全落入后，余额使用单个 Tween 启动一次滚筒动画：只展示少量抽样中间值，不逐金币排队计数，前段快速掠过、末段减速并精确停在用户当前持有的金币总数。大额消费仍最多构建 `10` 个可见滚动步骤，在保持结束值和总时长正确的同时避免大量 Tween 导致手机掉帧。飞行金币固定使用移动端小尺寸，不能被素材原始尺寸撑大或飞出结果页。滚筒停止后小狮子恢复随机庆祝动作，整段结算动画按奖励数量控制在约 `2.15-3.3s`；金币已在结算状态写入时落盘，动画不得延迟或重复发放。
- 当前飞币起点不再使用固定 `0.45s` 页面计时：GOOD 等待小狮子入场完成，EXCELLENT 等待撒花与入场都完成；条件满足后立即进入上述飞币阶段。
...
- EXCELLENT 结算播放延迟加载的柔和欢快音乐 `result_cheerful.wav`，旋律使用低亮度木质与卡林巴音色，禁止尖锐玻璃高频；GOOD 结算不播放该音乐。EXCELLENT 使用 32 枚花瓣、总时长约 `4.8s`。花瓣统一使用可运行时染色的白色中性矢量素材 `assets/ui/petal_neutral.svg`，素材通过浅灰明暗、中心叶脉、柔和高光，以及右侧和底部逐渐加深的局部边缘阴影保留轻微真实厚度；阴影不能形成整圈深色描边。每轮动画随机打乱完整多色花瓣色板，不能退化为单色或固定颜色顺序。结算页显示后必须等待嵌套 Container 完成布局，再以小狮子的实际全局中心建立落点；花瓣从屏幕顶部持续向下飘落，位置始终沿现有二次贝塞尔曲线推进，并只用横向缩放模拟柔和翻面，不得让翻面偏移曲线路径。花瓣穿过标题区域时保持清晰可见，只在最后约 `14%` 路程、已经进入小狮子主体附近后逐渐淡出消失，不再把标题或金币图标作为散花落点。最后一枚花瓣结束后不得增加人工停顿，立即开始从小狮子到金币图标的飞币动画；金币到账使用低频、短尾音的阻尼落槽声，禁止高频双金属长鸣。最后一枚金币到达后立即启动数字滚筒，滚轮不再播放固定时长的连续棘轮录音，而是由数字动画在每个真实可视步结束时发出一次短促、柔和的系统选择器式刻度音，刻度间隔跟随 Tween 逐步减速；最终一格由独立的低频锁定音收尾，不与刻度音叠加。三段视觉动作严格按“撒花 → 飞币 → 数字滚动”串行衔接，音乐本身在撒花后主动降低编配密度，为金币音效留出空间。页面提前关闭时统一取消尚未开始的阶段、正在运行的 Tween 和音乐。所有音频素材由 `tools/generate_audio_sfx.py` 使用固定种子确定性生成，不依赖外部版权资源。
...
| 展示关卡基础奖励 | 1 / 2 / 3 / 4 / 5 | `CoinRewardPolicy.base_reward_for_display_level()`、`SIZE_UNLOCK_DISPLAY_LEVELS` | 对应第 1 / 11 / 80 / 160 / 240 关起的阶段 |
...
| 首页拼块入口 | 完成新人流程后常驻；第 11 关解锁 | `_start_home_composite_flow()`、`LevelDirector.minimum_display_for_size()`、`CompositeLevelDirector` | 未满足普通关卡 6x6 size 条件时点击弹窗提示所需关卡且不创建现场；解锁后由普通导演先推荐 size/difficulty class，再按该 size 的独立多项后验采样 Simple/Medium/Hard；退出后恢复主线现场并保留拼块金币净变化 |
...
| 启动 Splash 默认候选 | 方案 A：小狮子点亮棋盘，`0.85-1.40s` | `scripts/overlays/splash_overlay.gd`（待实现） | 冷启动播放一次；方案 B 用于拼块入口，方案 C 用于低动效或老玩家变体 |
...
1. 运行核心冒烟测试。
2. 运行新手教程冒烟测试。
3. 手动验证首页“开始关卡 / 拼块玩法 / 新人流程”三个主要入口；第 10 关及以前拼块入口应显示第 11 关解锁并弹窗拦截，第 11 关起应进入 6x6 拼接阶段，返回首页后主线现场与资源不变。
...
12. 如果修改了存档结构，必须验证旧存档兼容。
13. 手动确认首页、关卡页、弹窗和关卡编辑器中的中文均正常显示，不出现缺字方框。
14. 至少完成 1 个 Simple 单/双区域、1 个 Medium 双区域和 1 个 Hard 三区域的 6x6 复合关卡，验证首次动画、Help 重播、顶部单行待放置区左右滚动、3-4 个块可见、颜色发光边框、自动排序、放置撤回、直找、提示浮窗、唯一解转换和中途恢复。
15. 同一本地日期连续新开 4 个首页拼块局：前 3 局免费；第 3 局共享结算页的“下一局”按钮显示 2 金币价格，点击后在该结算页上方弹出消费确认，取消仍停留结算页，确认后才进入第 4 局并在新关卡页播放从扣除前余额到扣除后余额的动画。中途退出恢复不重复收费。分别验证 Good 结算返还 2 金币、Excellent 结算返还 4 金币；修改为下一本地日期后再次获得 3 个免费局。
16. 更新本文档对应模块。
17. Splash 接入后，分别验证新用户进入教程、已有存档恢复原流程、资源慢加载时不会空白或无限循环、静音/减少动画/跳过入口生效，并确认 Splash 不修改任何正式游戏存档。
