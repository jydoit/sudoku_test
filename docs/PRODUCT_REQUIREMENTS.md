...
用户目标：明确当前在 color king 主界面，看到主视觉和最核心入口，一键进入当前关卡；开发/验收时可快速模拟新人流程。
...
- 顶部栏包含返回首页、金币图标与余额、独立红心体力槽和“选关”按钮；底部操作区同一行显示清除、皇冠直找和提示三个道具；不显示设置、排行榜或其它资源入口。
- 关卡标题只显示关卡编号，不显示关卡名称。
...
- 顶部编辑、设置和底部广告入口已从关卡页移除；顶部资源栏显示金币与本关红心并保留“选关”入口，底部操作区显示清除、皇冠直找和提示按钮。
...
- 页面在移动端宽度下按钮和棋盘不溢出。
- 关卡页顶部显示金币余额和本关红心，底部显示清除、皇冠直找和提示按钮，不显示排行榜、编辑、设置和广告。
...
- 双击空格或普通排除 X 尝试标记皇冠。
- 双击标准答案格放置张嘴开心笑、眼睛带高光的狮子皇冠；双击非标准答案格会显示嘴角向下的苦笑狮子，并叠加不可修改的红色 X 徽标，同时扣除 1 个红心。
...
- 清除为永久免费道具，只清除普通排除 X 和普通皇冠；开局皇冠、皇冠直找产生的提示皇冠和错误红色 X 保持锁定，清除结果可以撤销。
...
- 双击正确答案格显示皇冠并触发反馈。
- 双击非答案格显示红色 X、扣除 1 个红心并触发反馈；红色 X 是系统确认的排除信息，单击、滑动和清空棋盘都不能修改。
- 点击皇冠直找按钮会直接找到一个锁定皇冠并消耗 1 次；次数为 0 时不可继续使用。
- 点击清除会移除普通 X 和普通皇冠，不核销金币、不恢复免费道具次数，并保留所有锁定格。
...
- `_use_hint()` 执行提示消耗、设置可操作范围和更新教练文本。
...
| 清除价格 | 0 | `_clear_board()` | 永久免费，仅清理普通 X 和普通皇冠 |
...
| 全局 UI 字体 | Noto Sans SC Regular | `assets/fonts/NotoSansSC-Regular.ttf`、`project.godot` | 字体随项目打包，所有中文界面不依赖系统字体回退 |
