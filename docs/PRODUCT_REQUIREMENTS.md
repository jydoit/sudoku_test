...
- 成功结算使用原创生成的 `result_lion_nature_v2.wav` 背景短曲；教程完成使用更轻、更短的 `result_tutorial_soft.wav`；失败和死局不复用成功音乐，只保留进入失败前的错误/死局反馈与安静停顿。成功音乐约 `4.2s`，采用卡林巴、木琴、轻木质打击和柔和自然氛围，旋律按照“小狮子出现时上扬—金币飞入时爬升—中段回落—余额滚动时再次抬升—最终余额停止时落在温暖和弦”形成明确起伏，不得呈现电子舞曲感。母带使用柔和动态压缩降低短暂闪光音的峰均比，让木琴与卡林巴主体平均响度稳定；峰值约 `-7dBFS`，运行时目标增益为 `-9dB`、淡入约 `0.35s`。素材由 `tools/generate_audio_sfx.py` 使用固定种子确定性生成，不依赖外部音乐版权，且不得随主场景预加载，只在相应结果状态首次出现时加载并缓存。金币飞入最多播放三次轻落点声，全部金币到达后播放一次连续滚筒声，余额停止时播放一次确认声。最终余额停止后音乐采用两段式淡出：第一段必须从当前音量继续下降约 `7dB`，不得反向增响，再用约 `0.6s` 消失；页面提前关闭时同样触发淡出。
