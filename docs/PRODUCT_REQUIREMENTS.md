...
- 每个正式关卡有 3 个红心；红心为 0 时，本关失败并显示关卡失败页。
...
- 失败机制已接入正式关卡：每关 3 个红心，非标准答案皇冠尝试扣红心并标记红色 X，红心归零显示失败页。
...
- 顶部栏包含返回首页和“选关”按钮；底部操作区同一行显示本关红心、皇冠直找按钮和提示按钮；不显示金币、设置或其它右上角按钮。
- 关卡标题只显示关卡编号，不显示关卡名称。
- “选关”按钮打开关卡选择弹窗，用户确认后直接切换到所选正式关卡。
- 教练提示区显示关卡教程或当前提示解释。
- 棋盘占据主要空间，正式关卡底部保留本关红心、皇冠直找按钮和提示按钮。
- 底部不显示广告占位。
...
- 顶部编辑、设置和底部广告入口已从关卡页移除；底部操作区显示本关红心、皇冠直找按钮和提示按钮，顶部保留“选关”入口用于切换正式关卡。
...
- 页面在移动端宽度下按钮和棋盘不溢出。
- 关卡页显示本关红心、皇冠直找按钮和提示按钮，不显示金币、排行榜、编辑、设置和广告。
...
- 双击空格或普通排除 X 尝试标记皇冠。
- 双击标准答案格放置皇冠；双击非标准答案格会标记不可修改的红色 X，并扣除 1 个红心。
- 每个正式关卡进入时重置为 3 个红心，错误皇冠尝试扣 1 个红心；红心为 0 时锁定棋盘并弹出关卡失败页。
- 皇冠直找按钮与红心、提示按钮同一行显示；每个用户初始 3 次，点击后直接把一个未找到的标准答案格标记为锁定皇冠，次数用完后按钮禁用，后续预留激励广告补充次数。
...
- 保存当前关卡、棋盘状态、金币、提示次数、皇冠直找次数、已完成关卡、完成状态和即时纠错开关。
...
| 每关红心 | 3 | `INITIAL_HEART_COUNT` | 正式关卡每次进入/重试的生命数 |
