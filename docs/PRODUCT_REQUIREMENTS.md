...
- 结算页只展示真实产生的金币奖励，不展示没有后续用途的“获得皇冠 +1”虚拟奖励；奖励行只保留金币图标和右侧金色大号账户余额，不再显示 `coins / 金币奖励` 文字。关卡顶部余额字号为 `24px`，数字区最小为 `62 x 44px`；结算页余额字号为 `32px`，数字区最小为 `104 x 52px`并保留上下字体安全空间，多位数时继续自动增宽。页面淡入后停顿约 `0.45s`，数字先显示本次奖励到账前的账户余额；小狮子配合挥手/抛金币动作，将与奖励数量一致的金币沿交错弧线逐枚抛向奖励行的金币图标，每枚金币落入时目标图标轻微回弹，裁切窗口内的余额同步向上滚动 `1`，最终停在用户当前持有的金币总数。多枚金币密集到达时，数字滚动请求按到达顺序排队，不得终止或复位前一格动画；大额消费最多构建 `10` 个可见滚动步骤，在保持结束值和总时长正确的同时避免大量 Tween 导致手机掉帧。飞行金币固定使用移动端小尺寸，不能被素材原始尺寸撑大或飞出结果页。全部到账后小狮子恢复随机庆祝动作。总时长按奖励数量控制在约 `1.35-2.8s`，金币已在结算状态写入时落盘，动画不得延迟或重复发放。
