...
1. `assembly_intro`：首次体验时进入一次真实拖放教学。系统从当前合法完整布局中选择一个尚未放置的方块及其正确落点，只高亮该方块与对应凹槽；玩家完成一次真实拿取、吸附与放置后进入自由拼接。教学可跳过，返回首页会取消教学且不会误记为已完成；已经看过时直接进入拼接，Help 中可重播但不修改历史完成状态。
...
- 拼块首次引导不使用自动代演、假滑动或假放置。引导期间保留“返回首页”和引导内“跳过”，临时禁用 Help、设置、选关及底部道具，避免多个交互层抢占输入；失败释放不惩罚玩家，而是恢复同一方块到同一正确落点的手势提示。
...
- `scripts/assembly_view.gd` 使用 `assets/ui/block_tile_neutral.png` 作为可运行时染色的单格纹理，拼装立体锁定区域、托盘方块、拖动预览和棋盘落块；同时绘制凹槽、深色横向待放置区、颜色发光 slot、提示浮窗、合法放置后的落点光环、撤回到托盘后的 slot 回弹反馈和压平过场，并处理鼠标与单指触控的横向滚动/拖放仲裁。
...
- `scripts/dialogs/settings_dialog_content.gd` 统一承载语言、音乐、音效和震动反馈开关；`main.gd` 负责设置入口、布局方向切换以及 `selectedLanguage / musicEnabled / sfxEnabled / hapticsEnabled` 的存取。三个声音与震动开关彼此独立，旧存档缺失字段时默认开启。
...
- 音乐、音效和震动反馈设置重新进入游戏后保持；关闭音效不影响音乐，关闭音乐不影响操作音效，关闭震动后棋盘错误与通关节点不得调用手机震动。
...
- 所有结算状态均不播放背景音乐，也不打包历史结算背景音乐素材。EXCELLENT 撒花开始时只播放一次约 `1.75s` 的非旋律纸片飘散与闪光音效 `petal_scatter.wav`；30 枚花瓣完整播放约 `5.3s` 后，先保留约 `0.24s` 的安静停顿，再进入金币阶段。金币飞入最多在首枚、中段和末枚落点播放三次清晰的双金币金属碰撞声；最后一枚完全到达后必须再静默约 `0.28s`，随后才同时启动数字滚筒和约 `1.08s` 的减速金属棘轮声，余额精确停止时播放一次短金属锁止音。撒花、金币飞入和数字滚动必须串行执行，任何阶段不得提前启动下一阶段或让三组声音叠加。页面提前关闭时统一取消尚未开始的阶段和正在运行的 Tween。所有素材由 `tools/generate_audio_sfx.py` 使用固定种子确定性生成，不依赖外部版权资源。
