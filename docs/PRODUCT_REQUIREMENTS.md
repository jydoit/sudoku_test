...
- `play_cell_feedback()`、`play_wrong_feedback()` 和 `play_guide_feedback()` 分别提供普通放置/标记弹跳、错误皇冠短促晃动和提示柔和光环反馈。
...
- 当皇冠数量等于目标数且无冲突时通关。
- 找到最后一只小狮子后，先在关卡棋盘内播放约 `2.3s` 的分段完成庆祝：最后落下的小狮子先缓慢上升、在最高处短暂停留、再缓慢落回，其余已找到的小狮子按与其距离由近到远依次执行同样的上浮响应，最远处的小狮子也必须完整完成动作；加宽的金色径向扫光用约 `1.4s` 覆盖棋盘，并在扫过后保留暖金余辉、金色外框和星点收尾，在金色状态到达峰值时补充一次轻触感。约 `2.52s` 后才进入结算页。整段效果由单一时间轴驱动，不为每个格子创建独立 Tween；局内庆祝不能与结算金币和花瓣动画重叠。
...
- `scripts/pages/result_page.gd` 是独立的全屏结果页，同时承载成功、失败、拼块死局和教程完成四种视觉状态。页面通过 `present_success / present_failure / present_deadlock / present_tutorial_complete` 接收语义数据，自行生成本地化文案并管理金币飞入、余额滚动、小狮子逐帧庆祝和花瓣动画；奖励计算与完成/失败统计由 `scripts/services/run_result_service.gd` 负责，`scripts/main.gd` 只处理结算后的导航。成功结算的角色展位显示一只开心站立的橘色皇冠狮子，每次进入结算页时随机选择自然挥手、逐步吐舌头或眨眼做鬼脸动画。每组循环使用 9-12 个动作关键帧，并使用不等长帧时长与动作停顿避免机械循环；角色容器的缩放、旋转和中心位置始终固定，不允许通过呼吸缩放或摇摆造成前后抖动。
...
- 拼块整盘完成音效与 `AssemblyView.play_flatten_transition()` 同时开始，包含落位、沉降和转换闪光层；最终直找补齐整盘时同样必须执行该完成音效。
...
- 正确选择皇冠时，小狮子执行约 `0.72s` 的两段开心弹起并出现短暂金色亮点；弹起过程中头像由普通微笑平滑过渡到弯月笑眼、嘴角上扬的开心表情，短暂停留后回到稳定皇冠头像，不得闪切。错误选择时，已有委屈表情必须在红色 X 上方清晰停留后淡出，并以约 `0.82s` 的进入、下沉、退场节奏和扣心反馈同步，不能只显示红 X 或只左右抖动。
- EXCELLENT 结算播放延迟加载的柔和欢快音乐 `result_cheerful.wav`，旋律使用低亮度木质与卡林巴音色，禁止尖锐玻璃高频；GOOD 结算不播放该音乐。EXCELLENT 使用 32 枚花瓣、总时长约 `4.8s`。结算页显示后必须等待嵌套 Container 完成布局，再以小狮子的实际全局中心建立落点；花瓣从屏幕顶部持续向下飘落，穿过标题区域时保持清晰可见，只在最后约 `14%` 路程、已经进入小狮子主体附近后逐渐淡出消失，不再把标题或金币图标作为散花落点。最后一枚花瓣结束后不得增加人工停顿，立即开始从小狮子到金币图标的飞币动画；金币到账使用低频、短尾音的阻尼落槽声，禁止高频双金属长鸣。最后一枚金币到达后立即启动数字滚筒，滚轮不再播放固定时长的连续棘轮录音，而是由数字动画在每个真实可视步结束时发出一次短促、柔和的系统选择器式刻度音，刻度间隔跟随 Tween 逐步减速；最终一格由独立的低频锁定音收尾，不与刻度音叠加。三段视觉动作严格按“撒花 → 飞币 → 数字滚动”串行衔接，音乐本身在撒花后主动降低编配密度，为金币音效留出空间。页面提前关闭时统一取消尚未开始的阶段、正在运行的 Tween 和音乐。所有素材由 `tools/generate_audio_sfx.py` 使用固定种子确定性生成，不依赖外部版权资源。
