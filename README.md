...
- 棋盘底色为白色；只有棋盘最外缘使用深色边框，不同颜色区域之间无深色分割线，所有格子之间保留白色格间距，并为 10 个颜色加入轻量底纹
- 正式关卡底部只显示“小狮子”和“提示”两个道具按钮
- 正式关卡按标准答案判定小狮子尝试；非答案格会扣除红心并标记不可修改的红色 X
- 小狮子视觉统一使用戴皇冠的小狮子贴图，正确小狮子、提示小狮子、开局小狮子和结算页使用同一角色体系
- 小狮子按钮：用户初始 3 次，点击后直接找到一个小狮子，免费次数用完后按当前关卡价格消耗金币
...
- 棋盘视觉：白色底板、整数像素对齐，10 色明亮色板，棋盘外缘使用深色边框，不同颜色区域之间无深色分割线，所有格子之间保留白色格间距，并带轻量底纹
- 行、列、颜色区域和八方向相邻的即时冲突提示
- 正式关卡按展示关卡序号分配红心：第 1-10 关 3 个，第 11-30 关 2 个，第 31 关起 1 个；红心归零时显示失败页
- 小狮子直找道具：初始 3 次，直接放置一个锁定小狮子；免费次数耗尽后显示金币价格
